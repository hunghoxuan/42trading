...
export default function SymbolTogglePicker({
  ...
  const symbolComboOptions = useMemo(
    () =>
  ...
  const controlsNode = (
    ...
    >
      <label className="stack-layout" style={{ gap: 4 }}>
        <FormComboSelect value={sourceGroup} onChange={(e) => setSourceGroup(e.target.value)}>
      ...
      <label className="stack-layout" style={{ gap: 4 }}>
        <FormComboSelect
          value=""
          onChange={(event) => {
            const nextSymbol = normalizeSymbol(event.target.value);
            if (!nextSymbol) return;
            setSearch(nextSymbol);
            handleToggle(nextSymbol);
          }}
          searchable
