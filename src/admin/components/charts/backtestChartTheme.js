...
export function resolveTradeFocusedWindow(
  ...
  const requestedVisibleBars = Math.max(
    40,
    Math.round(Number(visibleBarsCount) || 0) || 300,
  );
  ...
  const contextPad = Math.max(
    20,
    Math.min(140, Math.round(anchorSpanBars * 3.5)),
  );
  const targetWindowBars = Math.max(
    40,
    Math.min(requestedVisibleBars, anchorSpanBars + contextPad * 2),
  );
  const desiredLeftPad = Math.max(
    12,
    Math.round((targetWindowBars - anchorSpanBars) / 2),
  );
  const desiredRightPad = Math.max(
    12,
    targetWindowBars - anchorSpanBars - desiredLeftPad,
  );
