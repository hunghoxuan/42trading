  ...
  return (
    <div className="ai-hub-pane">
      ...
      <div className="ai-hub-list">
        {results.length ? (
          results.map((entry) => {
            const display = buildNotificationDisplay(entry);
            const duration = formatEntryDuration(entry);
            return (
              <article key={entry.requestId} className="ai-hub-card">
                <div className="ai-hub-card__top">
                  <span className={isErrorEntry(entry) ? "msg-error" : ""}>
                    {display.title}
                  </span>
                ...
                {display.detail ? (
                  <div className="ai-hub-card__title">{display.detail}</div>
                ) : null}
                {duration ? (
                  <div className="ai-hub-card__meta">Duration {duration}</div>
                ) : null}
              </article>
            );
          })
...
export default function AiChatDock({
  ...
  const canUseCodex =
    IS_LOCAL_DEV || String(authUser?.role || "").toLowerCase() === "system";
  ...
  const contextSummary = useMemo(
    () => ({
      ...
      userRole: String(authUser?.role || "").trim().toLowerCase() || "user",
    }),
    [authUser?.role, pathname, routeContext],
  );
        ...
        <>
          ...
          >
            ...
            <GroupButtons
              ...
              selectionMode="single"
              onChange={([nextTab]) => setActiveTab(nextTab || "ai")}
              border_type="multiple"
