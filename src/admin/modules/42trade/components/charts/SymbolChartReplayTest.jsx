...
function buildReplayCompletedBarsForTf({
  bars = [],
  tf = "",
  replayClockTimeSec = null,
  replayStartTimeSec = null,
  ...
  const visibleCount = Math.max(20, Math.min(maxBars, completedBars.length));
  return completedBars.slice(-visibleCount);
}
...
function artifactItemToChartObject(item = {}, fallbackTf = "", barsByTf = null) {
  ...
  if (groupKey === "swings") {
    ...
    return swingConnection
      ? [swingConnection, swingLevelLine]
      : [swingLevelLine];
  }
...
export default function SymbolChart({
  ...
  externalArtifactObjectsByChartId = null,
  chartStrategies = EMPTY_ARRAY,
  extraRequestedTimeframes = EMPTY_ARRAY,
  liveBars = true,
  bootstrapLiveBarsOnMount = false,
  replayArtifactExperimentMode = "default",
  replayArtifactExperimentButtons = EMPTY_ARRAY,
  onReplayArtifactExperimentModeChange = null,
  replayTestCommand = null,
  replayTestSpeedMs = null,
  hideInternalReplayControls = false,
}) {
  ...
  const onReplayProgressChangeRef = useRef(onReplayProgressChange);
  ...
  useEffect(() => {
    onReplayProgressChangeRef.current = onReplayProgressChange;
  }, [onReplayProgressChange]);
  useEffect(() => {
    const nextSpeedMs = Number(replayTestSpeedMs);
    if (!Number.isFinite(nextSpeedMs) || nextSpeedMs <= 0) return;
    setLocalReplaySpeedMs((prev) =>
      prev === nextSpeedMs ? prev : Math.max(100, nextSpeedMs),
    );
  }, [replayTestSpeedMs]);
  ...
  const isGenericReplayActive = !hasExternalReplayConfig && mode === REPLAY_MODE && localReplayPlaying;
  const disableAutomaticCacheRefresh =
    hideInternalReplayControls === true && (isReplayMode || localReplayPlaying);
  const replayEnabledInChart = Boolean(
    effectiveReplayConfig?.enabled &&
    effectiveReplayConfig?.playing &&
    activeMode === replayActiveMode,
  );
  const effectiveReplayArtifactExperimentMode = useMemo(() => {
    const raw = String(replayArtifactExperimentMode || "default")
      .trim()
      .toLowerCase();
    return ["replay1", "replay2"].includes(raw) ? raw : "default";
  }, [replayArtifactExperimentMode]);
  const isReplayArtifactExperimentActive =
    replayEnabledInChart && effectiveReplayArtifactExperimentMode !== "default";
  ...
  const isBacktestChartReplay = hasExternalReplayConfig
    ? replayEnabledInChart
    : isGenericReplayActive;
  ...
  const replayCanStart = isTradeAnchoredReplay
    ...
    : primaryReplayBars.length > 1;
  const replayButtonsDisabled = status === "LOADING" || pendingMode === "cache";
  const replayTestCommandNonceRef = useRef(0);
  const replayDebugSummary = useMemo(
    () =>
      [
        `mode=${String(mode || "")}`,
        `active=${String(activeMode || "")}`,
        `playing=${effectiveReplayConfig?.playing ? "1" : "0"}`,
        `enabled=${effectiveReplayConfig?.enabled ? "1" : "0"}`,
        `backtest=${isBacktestChartReplay ? "1" : "0"}`,
        `cursor=${Number.isFinite(Number(chartReplayCursorIndex)) ? Number(chartReplayCursorIndex) : -1}`,
        `start=${Number.isFinite(Number(replayWindowIndices.startIndex)) ? Number(replayWindowIndices.startIndex) : -1}`,
        `end=${Number.isFinite(Number(replayWindowIndices.endIndex)) ? Number(replayWindowIndices.endIndex) : -1}`,
        `ready=${replayWindowIndices.ready ? "1" : "0"}`,
        `bars=${Array.isArray(primaryReplayBars) ? primaryReplayBars.length : 0}`,
      ].join(" | "),
    [
      activeMode,
      chartReplayCursorIndex,
  ...
  const triggerReplayForArtifactMode = useCallback(
    (nextMode = "default") => {
      const normalizedMode = ["replay1", "replay2"].includes(String(nextMode || "").trim().toLowerCase())
        ? String(nextMode || "").trim().toLowerCase()
        : "default";
      if (typeof onReplayArtifactExperimentModeChange === "function") {
        onReplayArtifactExperimentModeChange(normalizedMode);
      }
      const isPlaying = Boolean(effectiveReplayConfig?.playing);
      const isCurrentMode = effectiveReplayArtifactExperimentMode === normalizedMode;
      if (!hasExternalReplayConfig) {
        if (mode !== REPLAY_MODE && !isPlaying) {
          enterReplayMode();
          window.setTimeout(() => {
            setLocalReplayPlaying((prev) => !prev);
          }, 0);
          return;
        }
        if (isPlaying) {
          if (normalizedMode === "default" || isCurrentMode) {
            setLocalReplayPlaying(false);
          }
          return;
        }
        setLocalReplayPlaying(true);
        return;
      }
      if (isPlaying) {
        if (isCurrentMode) {
    ...
    [
      enterReplayMode,
      effectiveReplayArtifactExperimentMode,
      effectiveReplayConfig,
      hasExternalReplayConfig,
      mode,
      onReplayArtifactExperimentModeChange,
    ],
  );
  useEffect(() => {
    const commandNonce = Number(replayTestCommand?.nonce);
    if (!Number.isFinite(commandNonce) || commandNonce <= 0) return;
    if (replayTestCommandNonceRef.current === commandNonce) return;
    replayTestCommandNonceRef.current = commandNonce;
    const requestedMode = String(replayTestCommand?.mode || "default")
      .trim()
      .toLowerCase();
    const normalizedMode = ["replay1", "replay2"].includes(requestedMode)
      ? requestedMode
      : "default";
    if (typeof onReplayArtifactExperimentModeChange === "function") {
      onReplayArtifactExperimentModeChange(normalizedMode);
    ...
    setPendingMode(null);
    setChartReplayCursorIndex(-1);
    triggerReplayForArtifactMode(normalizedMode);
  }, [
    onReplayArtifactExperimentModeChange,
    replayTestCommand?.mode,
    replayTestCommand?.nonce,
    triggerReplayForArtifactMode,
  ]);
  ...
  const replayArtifactEngineStateRef = useRef({});
  ...
  useEffect(() => {
    ...
    (requestedSortedTfs || []).forEach((tf) => {
      ...
      if (!(Array.isArray(loadedBars) && loadedBars.length > 0)) return;
  ...
  const effectiveArtifactObjectsByChartId =
    externalArtifactObjectsByChartId &&
    typeof externalArtifactObjectsByChartId === "object" &&
    !Array.isArray(externalArtifactObjectsByChartId)
      ? externalArtifactObjectsByChartId
      : artifactObjectsByChartId;

  const artifactPanelGroups = useMemo(() => {
    const groups = new Map();
    for (const [chartId, items] of Object.entries(effectiveArtifactObjectsByChartId || {})) {
  ...
  }, [effectiveArtifactObjectsByChartId, cleanSym]);

  const artifactPanelTimeframes = useMemo(() => {
    const groups = new Map();
    for (const items of Object.values(effectiveArtifactObjectsByChartId || {})) {
      for (const item of Array.isArray(items) ? items : []) {
  ...
  }, [effectiveArtifactObjectsByChartId, artifactTfVisibility]);
  ...
  const loadReplayArtifactsForTf = useCallback(
    async (tf, replayMode = "default") => {
      const tfKey = String(tf || "").trim().toLowerCase();
      if (!tfKey || !cleanSym || replayMode === "default") return null;
      const chartId = `${cleanSym}-${tfKey}`;
      const chartScopeKey = `${chartId}:${replayMode}`;
      const loadedBars = Array.isArray(getTimeframeValue(master?.bars, tfKey))
        ? getTimeframeValue(master?.bars, tfKey)
        : [];
      if (!loadedBars.length || !Number.isFinite(Number(effectiveReplayBarsClockTimeSec))) {
        return null;
      }
      const replaySourceBars =
        replayMode === "replay1"
          ? buildReplayCompletedBarsForTf({
              bars: loadedBars,
              tf: tfKey,
              replayClockTimeSec: effectiveReplayBarsClockTimeSec,
              replayStartTimeSec: effectiveReplayStartTimeSec,
          ...
          : Array.isArray(replayBarsByTf?.[tfKey])
            ? replayBarsByTf[tfKey]
            : [];
      const requestKey = [
        cleanSym,
        tfKey,
        replayMode,
        Number(effectiveReplayBarsClockTimeSec) || 0,
        replaySourceBars.length,
        Number(replaySourceBars[0]?.time) || 0,
        Number(replaySourceBars[replaySourceBars.length - 1]?.time) || 0,
      ].join("|");
      if (artifactRequestKeyRef.current[chartScopeKey] === requestKey) {
        return null;
      }
      artifactRequestKeyRef.current[chartScopeKey] = requestKey;
      const nextSeq = (artifactRequestSeqRef.current[chartScopeKey] || 0) + 1;
      ...
      const response = {
        ...
        start_time: Number(replaySourceBars[0]?.time) || null,
        end_time: Number(replaySourceBars[replaySourceBars.length - 1]?.time) || null,
        artifacts: buildClientChartArtifactEnvelope(replaySourceBars, tfKey, {
          replaceExisting: true,
        }),
      };
      if (artifactRequestSeqRef.current[chartScopeKey] !== nextSeq) return null;
  ...
  useEffect(() => {
    if (typeof onReplayProgressChangeRef.current !== "function") return;
    onReplayProgressChangeRef.current({
      enabled: Boolean(effectiveReplayConfig?.enabled),
      playing: Boolean(effectiveReplayConfig?.playing),
      progressIndex: replayProgressIndex,
      progressTotal: replayProgressTotal,
      clockTimeSec: Number.isFinite(Number(replayClockTimeSec))
        ? Number(replayClockTimeSec)
        : null,
      barsVisible: Number.isFinite(Number(replayBarsByTf?.[primaryReplayTf]?.length))
        ? Number(replayBarsByTf[primaryReplayTf].length)
        : 0,
      replayArtifactMode: effectiveReplayArtifactExperimentMode,
    });
  }, [
    effectiveReplayArtifactExperimentMode,
    effectiveReplayConfig?.enabled,
  ...
  useEffect(() => {
    ...
    artifactRequestSeqRef.current = {};
    replayArtifactLoadKeyRef.current = "";
    replayArtifactEngineStateRef.current = {};
  }, [
    cleanSym,
    effectiveReplayArtifactExperimentMode,
    effectiveReplayConfig?.runKey,
  ...
  useEffect(() => {
    ...
    if (isReplayArtifactExperimentActive) return;
    ...
    const targetTfs = (sortedTfs || [])
      ...
      .filter((tf) => {
        const bars = getTimeframeValue(master?.bars, tf);
    ...
    cleanSym,
    isReplayArtifactExperimentActive,
    isBacktestChartReplay,
    loadArtifactsForTf,
    master?.bars,
  ...
  useEffect(() => {
    if (!isReplayArtifactExperimentActive || !cleanSym) return;
    const targetTfs = (sortedTfs || [])
      .map((tfRaw) => String(tfRaw || "").trim().toLowerCase())
      .filter(Boolean);
    if (!targetTfs.length) return;
    const replayBarsSourceByTf = {};
    targetTfs.forEach((tfKey) => {
      const loadedBars = Array.isArray(getTimeframeValue(master?.bars, tfKey))
        ? getTimeframeValue(master?.bars, tfKey)
        : [];
      if (!loadedBars.length || !Number.isFinite(Number(effectiveReplayBarsClockTimeSec))) {
      ...
      const replaySourceBars =
        effectiveReplayArtifactExperimentMode === "replay1"
          ? buildReplayCompletedBarsForTf({
              bars: loadedBars,
              tf: tfKey,
              replayClockTimeSec: effectiveReplayBarsClockTimeSec,
          ...
          : Array.isArray(replayBarsByTf?.[tfKey])
            ? replayBarsByTf[tfKey]
            : [];
      if (Array.isArray(replaySourceBars) && replaySourceBars.length) {
        replayBarsSourceByTf[tfKey] = replaySourceBars;
      }
    });
    const availableTfs = Object.keys(replayBarsSourceByTf);
    if (!availableTfs.length) return;
    const previousEngineState =
      replayArtifactEngineStateRef.current?.[effectiveReplayArtifactExperimentMode] ||
      createClientReplayArtifactEngineState();
    const { state: nextEngineState, envelopesByTf } = updateClientReplayArtifactEngineState(
      previousEngineState,
      replayBarsSourceByTf,
      {
        requestedTimeframes: availableTfs,
    ...
    availableTfs.forEach((tfKey) => {
      const chartId = `${cleanSym}-${tfKey}`;
      const replaySourceBars = replayBarsSourceByTf[tfKey];
      ...
      nextLoadStateByTf[tfKey] = {
        ...
        itemCount: nextObjects.length,
        loadedAt: Date.now(),
        replayMode: effectiveReplayArtifactExperimentMode,
        updateStrategy: String(stateForTf?.strategy || "full"),
      };
    });
    setArtifactObjectsByChartId((prev) => {
      const next = { ...(prev || {}) };
          ...
          effectiveReplayConfig?.enabled ? (
            <>
              ...
              <button
                ...
                className={
                  effectiveReplayConfig?.playing &&
                  effectiveReplayArtifactExperimentMode === "default"
                    ? "primary-button"
                    : "secondary-button"
                }
                onClick={() => triggerReplayForArtifactMode("default")}
                disabled={replayButtonsDisabled}
                title={
                  effectiveReplayConfig?.playing
                    ? "Pause replay for this chart set"
                    : replayCanStart
                      ? "Start replay for this chart set"
                      : "Replay start conditions are not ready yet in the test chart"
                }
              >
                {effectiveReplayConfig?.playing ? "Pause" : "Replay"}
              </button>
              {(Array.isArray(replayArtifactExperimentButtons)
                ? replayArtifactExperimentButtons
                : []
              ).map((button) => {
                const buttonKey = String(button?.key || "").trim().toLowerCase();
                if (!buttonKey || !["replay1", "replay2"].includes(buttonKey)) return null;
                const isActiveButton =
                  effectiveReplayConfig?.playing &&
                  effectiveReplayArtifactExperimentMode === buttonKey;
                return (
                  <button
                    key={buttonKey}
                    type="button"
                    className={isActiveButton ? "primary-button" : "secondary-button"}
                    onClick={() => triggerReplayForArtifactMode(buttonKey)}
                    disabled={replayButtonsDisabled}
                    title={
                      replayCanStart
                        ? String(button?.title || button?.label || buttonKey)
                        : "Replay start conditions are not ready yet in the test chart"
                    }
                  >
                    {String(button?.label || buttonKey)}
                  </button>
                );
              })}
              <span
                className="minor-text"
                title="Replay test debug"
                style={{
                  maxWidth: 520,
                  overflow: "hidden",
                  textOverflow: "ellipsis",
          ...
          >
            {displayTfs.map((tf) => {
              ...
              const artifactObjects = Object.entries(
                effectiveArtifactObjectsByChartId || {},
                  ...
                  {isLive ? (
                    ...
                    <>
                      <TradeSignalChart
                        ...
                        initialViewport={
                          disableViewportPersistence || anchorToTradeTime
                            ? null
                            : viewports?.[chartId] ||
                              savedTfViewportPrefs[chartId] ||
                              null
                        }
