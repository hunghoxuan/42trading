...
export function useRealtimeSymbolChartAdapter({
  ...
  return useMemo(() => {
                ...
                metadata: replayActive
                  ...
                  : {
                      ...
                      stream_connection_state:
                        String(liveState.connectionState || "idle").toLowerCase(),
                      stream_ever_connected: Boolean(liveState.everConnected),
                      stream_last_data_at:
                        Number.isFinite(Number(liveState.lastDataAt))
                          ? Number(liveState.lastDataAt)
                          : null,
                      stream_last_connected_at:
                        Number.isFinite(Number(liveState.lastConnectedAt))
                          ? Number(liveState.lastConnectedAt)
                          : null,
    ...
    liveState.connected,
    liveState.error,
    liveState.everConnected,
    liveState.lastPrice,
    liveState.lastConnectedAt,
    liveState.lastDataAt,
    liveState.lastUpdatedAt,
