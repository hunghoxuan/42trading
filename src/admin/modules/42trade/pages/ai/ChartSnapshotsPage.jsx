...
export default function ChartSnapshotsPage() {
  ...
  const chartUiConfigKey = useMemo(
    () =>
      `${String(provider || "").toUpperCase()}|${String(cfg.lookbackBars || "2000")}|${(Array.isArray(browserTfs) ? browserTfs : []).join(",")}`,
  ...
  const widgetTfs = useMemo(() => {
    ...
    const fallback = ["4h", "15m"];
    return (uniq.length ? uniq : fallback).slice(0, 4);
  }, [browserTfs]);
  ...
  const resolveCreatedId = (obj = {}, mode = "") => {
    const candidates = [
      obj?.sid,
      obj?.id,
      obj?.trade_id ?? obj?.signal_id,
      obj?.tradeId ?? obj?.signalId,
      obj?.trade_id,
      obj?.tradeId,
      obj?.created?.sid,
      obj?.created?.id,
      obj?.created?.trade_id ?? obj?.created?.signal_id,
      obj?.created?.trade_id,
      obj?.trade?.sid,
      obj?.trade?.id,
      obj?.trade?.sid ?? obj?.signal?.sid,
      obj?.trade?.id ?? obj?.signal?.id,
    ]
      .map((x) => String(x || "").trim())
      .filter(Boolean);
    if (!candidates.length) return null;
    return { kind: mode === "trade" ? "trade" : "draft", id: candidates[0] };
  };
  ...
  const saveWatchlistToDb = async (nextList) => {
    try {
      ...
      const persisted = mergeWatchlistIntoSymbolGroups(
        merged.data,
        merged.watchlist,
        result?.item?.data || merged.data,
      );
      setSymbolGroupsData(persisted.data);
      setCustomSymbolGroups(getCustomSymbolGroups(persisted.data));
      setWatchlist(persisted.watchlist);
    } catch (e) {
      ...
      >
                              ...
                              {filteredSyms.map((sym) => (
                                ...
                                >
                                  <SymbolChart
                                    ...
                                    initialBarsCount={Math.max(
                                      2000,
                                      resolveLookbackBarsValue(cfg.lookbackBars, "15m"),
                                    )}
        ...
        {!isTradeRoute &&
          ...
          (selectedSymbols.length <= 1 ||
            ...
            >
              <TradeDetailCard
                key={`${selectedSymbol || `response:${analyzeSessionId || tradeSidFromRoute || "detail"}`}::${widgetTfs.join("|")}`}
                mode="ai"
