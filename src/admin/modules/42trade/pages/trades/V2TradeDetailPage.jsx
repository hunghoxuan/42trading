...
function decisionBadgeColor(value) {
...
export default function TradeDetailPage() {
  ...
  const header = useMemo(() => {
    ...
    const aiDecision = detailPlan.trade_decision || "";
      ...
      aiBadges: (
        ...
        >
          ...
          >
            ...
            {aiDecision && (
              <span style={badgeS(decisionBadgeColor(aiDecision))}>
                {`Decision: ${aiDecision}`}
              </span>
            )}
            {aiAction && (
              <span style={badgeS(decisionBadgeColor(aiAction))}>
                {`Action: ${aiAction}`}
              </span>
            )}
          </div>
        ...
        >
          <TradeDetailCard
            ...
            metaItems={[
              ...
              {
                label: "TP",
                value: detailPlan.tp || trade.tp || "-",
                group: "identity",
              },
              {
                label: "SL",
                value: detailPlan.sl || trade.sl || "-",
                group: "identity",
              },
              {
                label: "RR",
                value: detailPlan.rr || trade.rr_planned || "-",
                group: "pnl",
              },
              {
                label: "Planned TP / SL",
                value:
                  resolveDisplayedPlannedPnl(trade, "sl") != null
                    ...
                    ? `${resolveDisplayedPlannedPnl(trade, "tp") != null ? `$${resolveDisplayedPlannedPnl(trade, "tp").toFixed(2)}` : "-"} / ${resolveDisplayedPlannedPnl(trade, "sl") != null ? `$${resolveDisplayedPlannedPnl(trade, "sl").toFixed(2)}` : "-"}`
                    : "-",
                renderValue:
                  resolveDisplayedPlannedPnl(trade, "sl") != null ||
                  resolveDisplayedPlannedPnl(trade, "tp") != null ? (
                    <span>
                      <span style={{ color: "#22c55e" }}>
                        {resolveDisplayedPlannedPnl(trade, "tp") != null
                          ? `$${resolveDisplayedPlannedPnl(trade, "tp").toFixed(2)}`
                          : "-"}
                      </span>
                      {" / "}
                      <span style={{ color: "#ef4444" }}>
                        {resolveDisplayedPlannedPnl(trade, "sl") != null
                          ? `$${resolveDisplayedPlannedPnl(trade, "sl").toFixed(2)}`
                          : "-"}
                      </span>
                    </span>
                  ) : null,
              ...
              },
              {
                label: "Last Sync",
                value:
                  (trade.updated_at || trade.metadata?.broker_data?.last_sync_at)
                    ? showDateTime(
                        trade.updated_at ||
                          trade.metadata?.broker_data?.last_sync_at,
                      )
                    : "-",
                group: "identity",
              },
              ...
              },
              {
                label: "Margin",
                value:
                  asNum(trade.broker_margin) != null
                    ? `$${asNum(trade.broker_margin).toFixed(2)}`
                    : "-",
                group: "sizing",
              },
              {
                label: "Vol",
                value:
                  asNum(trade.broker_volume) != null
                    ? `${Number(trade.broker_volume).toLocaleString()} units`
                    : "-",
                group: "sizing",
              },
              {
                label: "Lots",
                value:
                  asNum(trade.broker_lots) != null
                    ? `${asNum(trade.broker_lots).toFixed(2)} lots`
                    : "-",
                group: "sizing",
              },
              {
                label: "Commission",
                value:
                  asNum(trade.broker_commission) != null
                    ? `$${asNum(trade.broker_commission).toFixed(2)}`
                    : "-",
                group: "sizing",
              },
              {
                label: "Swap",
                value:
                  asNum(trade.broker_swap) != null
                    ? `$${asNum(trade.broker_swap).toFixed(2)}`
                    : "-",
                group: "sizing",
              },
              { label: "Trade SID", value: trade.sid || "-" },
              { label: "Trade ID", value: trade.trade_id || trade.sid || "-" },
              {
                label: "Direction",
              ...
              { label: "Est. Bars", value: detailPlan.estimated_bars || "-" },
              {
                ...
                value: formatTimeframe(
                  trade.trade_tf || trade.signal_tf || trade.tf || "-",
                ),
              },
              {
                label: "Chart TF",
