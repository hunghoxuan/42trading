...
import { getBrokerTicket } from "../../../../shared/utils/tradeRow";
import {
...
function normalizeTradeRealtimePatch(payload = {}) {
  ...
  const sid = String(source?.sid || payload?.sid || "").trim();
  if (!sid) return null;
  const patch = { ...source };
  if (payload?.execution_status != null && patch.execution_status == null) {
    patch.execution_status = payload.execution_status;
  }
  if (payload?.symbol && !patch.symbol) {
    patch.symbol = payload.symbol;
  }
  if (payload?.broker_pnl != null && patch.broker_pnl == null) {
    patch.broker_pnl = payload.broker_pnl;
  }
  if (payload?.pnl_realized != null && patch.pnl_realized == null) {
    patch.pnl_realized = payload.pnl_realized;
  }
  if (payload?.pnl != null) {
    if (patch.broker_pnl == null) patch.broker_pnl = payload.pnl;
    if (patch.pnl_realized == null) patch.pnl_realized = payload.pnl;
  }
  return patch;
}

function diffTradeFields(prevRow = {}, nextPatch = {}) {
  const changed = new Set();
  for (const [field, newVal] of Object.entries(nextPatch || {})) {
    if (field === "sid" || field === "symbol") continue;
    if (JSON.stringify(prevRow?.[field]) !== JSON.stringify(newVal)) {
      changed.add(field);
    }
  }
  return changed;
...
export default function TradesPage() {
  ...
  const tradeFlashTimeoutsRef = useRef(new Map());

  const applyTradeRealtimePatch = (patch) => {
    const normalizedPatch = normalizeTradeRealtimePatch(patch);
    const patchSid = String(normalizedPatch?.sid || "").trim();
    if (!patchSid) return;
    const diffBySid = new Map();
    setRows((prev) => {
      if (!Array.isArray(prev) || !prev.length) return prev;
      let matched = false;
      const next = prev.map((row) => {
        if (tradeKeyOf(row) !== patchSid) return row;
        matched = true;
        const changed = diffTradeFields(row, normalizedPatch);
        if (changed.size > 0) diffBySid.set(patchSid, changed);
    ...
    setSelectedTrade((prev) => {
      if (!prev) return prev;
      if (tradeKeyOf(prev) !== patchSid) return prev;
      return { ...prev, ...normalizedPatch };
    });
    if (diffBySid.size <= 0) return;
    ...
    const prevTimer = tradeFlashTimeoutsRef.current.get(patchSid);
    if (prevTimer) window.clearTimeout(prevTimer);
    const timeoutId = window.setTimeout(() => {
      setChangedFields((prev) => {
        const next = new Map(prev);
        const existing = next.get(patchSid);
        if (existing) {
          for (const field of diffBySid.get(patchSid) || []) {
            existing.delete(field);
          }
          if (!existing.size) next.delete(patchSid);
        }
        return next;
      });
      tradeFlashTimeoutsRef.current.delete(patchSid);
    }, 800);
    tradeFlashTimeoutsRef.current.set(patchSid, timeoutId);
  };
  ...
  useEffect(() => {
    const unsubscribe = realtimeClient.subscribe(
      ...
      (envelope) => {
        ...
        applyTradeRealtimePatch(envelope.data);
      },
      {
        onError: () => {},
      },
    ...
    return () => {
      unsubscribe();
      for (const timeoutId of tradeFlashTimeoutsRef.current.values()) {
        window.clearTimeout(timeoutId);
      }
      tradeFlashTimeoutsRef.current.clear();
    };
  }, []);
  ...
  useEffect(() => {
    const activeTradeId = String(tradeId || selectedTradeIdRef.current || "").trim();
    if (!activeTradeId) return undefined;
    const topic = `trade:${activeTradeId}`;
    const unsubscribe = realtimeClient.subscribe(
      topic,
      {},
      (envelope) => {
        const patch = normalizeTradeRealtimePatch(envelope?.data);
        if (!patch) return;
        const patchSid = String(patch?.sid || "").trim();
        if (!patchSid || patchSid !== activeTradeId) return;
        setRows((prev) => {
          if (!Array.isArray(prev) || !prev.length) return prev;
          let changed = false;
          const next = prev.map((row) => {
              ...
              >
                <TradeDetailCard
                  ...
                  header={(() => {
                    ...
                    const decisionText = detailPlan.trade_decision || "";
                    const actionText = detailPlan.skip_recommendation || "";
                            ...
                            {decisionText ? (
                              <span
                                ...
                                style={{
                                  ...
                                  fontWeight: 500,
                                  padding: "2px 6px",
                                  background: badgeColorFor(decisionText)
                                    ? `${badgeColorFor(decisionText)}20`
                                    : "transparent",
                                  color:
                                    badgeColorFor(decisionText) ||
                                    "var(--muted)",
                                  border: `1px solid ${badgeColorFor(decisionText) || "var(--border)"}`,
                                }}
                              >
                            ...
                            {actionText ? (
                              <span
                                ...
                                style={{
                                  ...
                                  fontWeight: 500,
                                  padding: "2px 6px",
                                  background: badgeColorFor(actionText)
                                    ? `${badgeColorFor(actionText)}20`
                                    : "transparent",
                                  color:
                                    badgeColorFor(actionText) ||
                                    "var(--muted)",
                                  border: `1px solid ${badgeColorFor(actionText) || "var(--border)"}`,
                                }}
                              >
                                {`Action: ${actionText}`}
                              </span>
                  ...
                  chart={{
                    ...
                    provider:
                      ...
                      "",
                    interval:
                      selectedTrade.trade_tf || selectedTrade.signal_tf || selectedTrade.chart_tf || "1h",
                    live: true,
                    entryPrice:
                  ...
                  metaItems={[
                    ...
                    },
                    {
                      label: "Planned TP / SL",
                      value: (() => {
                        const tpPnl = resolveDisplayedPlannedPnl(
                          selectedTrade,
                          "tp",
                        );
                        const slPnl = resolveDisplayedPlannedPnl(
                          selectedTrade,
                          "sl",
                        );
                        return tpPnl != null || slPnl != null
                          ? `${tpPnl != null ? `$${tpPnl.toFixed(2)}` : "-"} / ${slPnl != null ? `$${slPnl.toFixed(2)}` : "-"}`
                          : "-";
                      })(),
                      renderValue: (() => {
                        const tpPnl = resolveDisplayedPlannedPnl(
                    ...
                    },
                    {
                      label: "Broker Status",
                      value:
                        selectedTrade.metadata?.broker_data?.status ||
                        selectedTrade.execution_status ||
                        "-",
                      group: "identity",
                    },
                    ...
                    },
                    {
                      ...
                      value: (() => {
                        const margin =
                          ...
                          asNum(selectedTrade.metadata?.broker_data?.margin) ??
                          null;
                    ...
                    },
                    {
                      ...
                      value: (() => {
                        const vol =
                          ...
                          asNum(selectedTrade.metadata?.broker_data?.volume) ??
                          null;
                        return vol != null ? `${vol.toLocaleString()} units` : "-";
                      })(),
                      group: "sizing",
                    },
                    {
                      ...
                      value: (() => {
                        const lots =
                          asNum(selectedTrade.broker_lots) ??
                          asNum(selectedTrade.metadata?.broker_data?.lots) ??
                          null;
                        return lots != null ? `${lots.toFixed(2)} lots` : "-";
                      })(),
                      group: "sizing",
                    },
                    {
                      ...
                      value: (() => {
                        const commission =
                          ...
                          asNum(selectedTrade.metadata?.broker_data?.commission) ??
                          null;
                    ...
                    },
                    {
                      ...
                      value: (() => {
                        const swap =
                          ...
                          asNum(selectedTrade.metadata?.broker_data?.swap) ??
                          null;
                        return swap != null ? `$${swap.toFixed(2)}` : "-";
                      })(),
                      group: "sizing",
                    },
                    {
                    ...
                    {
                      label: "Trade TF",
                      value: formatTimeframe(selectedTrade.trade_tf || selectedTrade.signal_tf || "-"),
                      group: "source",
                    },
                    {
                      label: "Risk (%)",
