...
export function buildHeaderMeta({
  ...
  return {
    ...
    statusNode: <StatusDisplay status={status.cls} label={status.label} />,
  };
}
...
export function calcRrFromTrade(trade) {
  const entry = asNum(
    trade?.entry || trade?.target_price || trade?.entry_price || trade?.entry_price_raw,
  );
  const sl = asNum(trade?.sl || trade?.sl_price || trade?.sl_price_raw);
  ...
  const partials = Array.isArray(plan?.partial_tps)
    ? plan.partial_tps
    : Array.isArray(trade?.partial_tps)
      ? trade.partial_tps
      : [];
  let tp = asNum(trade?.tp || trade?.tp_price || trade?.tp_price_raw);
...
export function extractTradePlanFromTradeEntity(trade = {}) {
  const raw = (() => {
  ...
  const entry =
    ...
    asNum(trade?.entry || trade?.target_price || trade?.entry_price) ??
    asNum(raw?.entry ?? raw?.price);
  const tp =
    planPrimaryTp(effectivePlan) ?? asNum(trade?.tp || trade?.tp_price);
  const tp1 = planTpLevel(effectivePlan, 1) ?? asNum(trade?.tp1 ?? tp);
  const tp2 = planTpLevel(effectivePlan, 2) ?? asNum(trade?.tp2);
  const tp3 = planTpLevel(effectivePlan, 3) ?? asNum(trade?.tp3);
  const sl =
    asNum(effectivePlan?.execution_plan?.stop_loss?.price) ??
    asNum(effectivePlan?.sl ?? effectivePlan?.stop_loss) ??
    asNum(trade?.sl || trade?.sl_price);
  const rr =
    asNum(effectivePlan?.execution_plan?.risk_reward) ??
    asNum(effectivePlan?.rr ?? effectivePlan?.risk_reward) ??
    asNum(trade?.rr_planned) ??
    calcRrFromTrade(trade);
...
export const calcRrFromSignal = calcRrFromTrade;
export const extractTradePlanFromSignal = extractTradePlanFromTradeEntity;

export function validateTradePlan(plan = {}, opts = {}) {
  const entry = asNum(plan.entry);
  const tp = asNum(plan.tp1 ?? plan.tp);
  const tp2 = asNum(plan.tp2);
  const tp3 = asNum(plan.tp3);
  ...
  const targets = [tp, tp2, tp3].filter((value) => value != null);
  if (direction === "BUY") {
    if (!(tp > entry)) return "For BUY, TP must be greater than Entry.";
    if (!(sl < entry)) return "For BUY, SL must be lower than Entry.";
    if (tp2 != null && !(tp2 > tp))
      return "For BUY, TP2 must be greater than TP1.";
    if (tp3 != null && !(tp3 > (tp2 ?? tp)))
      return "For BUY, TP3 must be greater than TP2.";
    if (targets.some((value) => !(value > entry)))
      return "For BUY, all TP levels must be greater than Entry.";
  } else if (direction === "SELL") {
    if (!(tp < entry)) return "For SELL, TP must be lower than Entry.";
    if (!(sl > entry)) return "For SELL, SL must be greater than Entry.";
    if (tp2 != null && !(tp2 < tp))
      return "For SELL, TP2 must be lower than TP1.";
    if (tp3 != null && !(tp3 < (tp2 ?? tp)))
      return "For SELL, TP3 must be lower than TP2.";
    if (targets.some((value) => !(value < entry)))
      return "For SELL, all TP levels must be lower than Entry.";
  }
  return "";
}
