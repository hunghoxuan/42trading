...
function artifactPriceBounds(item = {}) {
  const top = Number(item?.price_top ?? item?.high ?? item?.priceHigh ?? item?.artifact_payload?.price_top);
  const bottom = Number(item?.price_bottom ?? item?.low ?? item?.priceLow ?? item?.artifact_payload?.price_bottom);
  const price = Number(item?.price ?? item?.level_price ?? item?.anchorPrice ?? item?.anchor_price ?? item?.artifact_payload?.price);
  if (Number.isFinite(top) && Number.isFinite(bottom)) {
    return { low: Math.min(top, bottom), high: Math.max(top, bottom) };
  }
...
function artifactTradeGroupWeight(groupKey = "", side = "BUY", kind = "tp") {
  ...
  const stopWeights = upperSide === "BUY"
    ? { pdl: 10, demand: 9, support: 9, ob: 9, bb: 9, fvg: 8, ifvg: 8, swings: 7, liquidity: 6, bos: 4, choch: 4 }
    : { pdh: 10, supply: 9, resistance: 9, ob: 9, bb: 9, fvg: 8, ifvg: 8, swings: 7, liquidity: 6, bos: 4, choch: 4 };
  const targetWeights = upperSide === "BUY"
    ? { pdh: 10, liquidity: 9, bos: 8, choch: 8, bb: 8, ob: 7, fvg: 7, ifvg: 7, swings: 6, resistance: 6, supply: 6 }
    : { pdl: 10, liquidity: 9, bos: 8, choch: 8, bb: 8, ob: 7, fvg: 7, ifvg: 7, swings: 6, support: 6, demand: 6 };
...
function resolveSuggestedTradeTimeframes(activeTf = "", selectedTfs = [], artifactItemsByTf = {}) {
  ...
  const activeTfSeconds = Math.max(1, Number(timeframeToSeconds(normalizedActiveTf)) || 0);
  const configuredTfs = (Array.isArray(selectedTfs) ? selectedTfs : [])
    .map((tf) => String(tf || "").trim().toLowerCase())
    .filter(Boolean);
  const fallbackArtifactTfs = Object.keys(artifactItemsByTf && typeof artifactItemsByTf === "object" ? artifactItemsByTf : {})
    .map((tf) => String(tf || "").trim().toLowerCase())
    .filter(Boolean);
  const candidateTfs = [...new Set([...configuredTfs, ...(normalizedActiveTf ? [normalizedActiveTf] : []), ...fallbackArtifactTfs])];
  if (!candidateTfs.length) return normalizedActiveTf ? [normalizedActiveTf] : [];
  ...
  const sortedScopedTfs = sortTimeframes(scopedTfs.length ? scopedTfs : candidateTfs, "desc");
...
function applyTradeStopBuffer(entry = null, stop = null, side = "BUY") {
  ...
  if (!Number.isFinite(numericEntry) || !Number.isFinite(numericStop)) return numericStop;
  const upperSide = String(side || "BUY").trim().toUpperCase();
...
function applyTradeTargetTrim({ entry = null, rawTarget = null, side = "BUY", risk = null, minRr = 1.5 }) {
  const numericEntry = Number(entry);
  const numericTarget = Number(rawTarget);
  const numericRisk = Number(risk);
  if (!Number.isFinite(numericEntry) || !Number.isFinite(numericTarget) || !(numericRisk > 0)) return numericTarget;
  const upperSide = String(side || "BUY").trim().toUpperCase();
...
function isReasonableArtifactPriceForTradeLevel(candidatePrice = null, entryPrice = null) {
  ...
  return Number.isFinite(ratio) && ratio > 0 && ratio >= 0.25 && ratio <= 4;
}

function buildSuggestedTradeLevels({ side = "BUY", entryPrice = null, referencePrice = null, activeTf = "", selectedTfs = [], artifactItemsByTf = {}, minRr = 1.5 }) {
  const entry = Number(entryPrice);
  ...
  tfList.forEach((tfRaw) => {
    ...
    tfItems.forEach((item) => {
      ...
      if (!isReasonableArtifactPriceForTradeLevel(low, entry) || !isReasonableArtifactPriceForTradeLevel(high, entry)) return;
      ...
      if (upperSide === "BUY") {
        if (low < anchor && low < entry) {
          ...
          if (distance > 0) {
            stopCandidates.push({ tfKey, groupKey, price: low, score: artifactTradeGroupWeight(groupKey, upperSide, "sl") * 100 + tfWeight * 10 - distance });
          }
        }
        if (high > anchor) {
          ...
          if (distance > 0 && rewardFromEntry > 0) {
            targetCandidates.push({ tfKey, groupKey, price: targetPrice, score: artifactTradeGroupWeight(groupKey, upperSide, "tp") * 100 + tfWeight * 10 - distance });
          }
        }
      } else {
        if (high > anchor && high > entry) {
          ...
          if (distance > 0) {
            stopCandidates.push({ tfKey, groupKey, price: high, score: artifactTradeGroupWeight(groupKey, upperSide, "sl") * 100 + tfWeight * 10 - distance });
          }
        }
        if (low < anchor) {
  ...
  const finalTarget = applyTradeTargetTrim({ entry, rawTarget: chosenTarget?.price ?? null, side: upperSide, risk, minRr: targetMinRr });
  return { sl: bufferedStop, tp: finalTarget };
}
