...
function validateScheduleRows(rows = []) {
  ...
  for (const row of activeRows) {
    ...
    if (!isValidTimeValue(row?.from) || !isValidTimeValue(row?.to)) {
      return `${dayLabel}: invalid time range.`;
    }
    const fromMinutes = timeToMinutes(row.from);
    const toMinutes = timeToMinutes(row.to);
    if (!Number.isFinite(fromMinutes) || !Number.isFinite(toMinutes)) {
      return `${dayLabel}: invalid time range.`;
    }
    if (fromMinutes >= toMinutes) {
      return `${dayLabel}: "From" must be earlier than "To".`;
    }
    if (
      !Number.isFinite(Number(row?.interval_seconds)) ||
      Number(row.interval_seconds) <= 0
    ) {
      return `${dayLabel}: interval is required.`;
    }
  }
  return "";
}
...
export default function CronPage() {
  ...
  useEffect(() => {
    ...
    async function loadTradeMeta() {
      try {
        ...
        setSources(Array.isArray(sourcesRes?.items) ? sourcesRes.items : []);
      } catch {
        if (cancelled) return;
        setAccounts([]);
        setSources([]);
  ...
  const handleRunCron = useCallback(async () => {
    ...
    setRunLoading(true);
    try {
      const res = await api.runCron(selectedCron.name);
          ...
          : typeof result?.captured === "number"
            ...
            : typeof result?.queued === "number"
              ...
              : typeof result?.triggered === "number"
                ...
                : "Completed";
      showToast({
        message: `Cron "${selectedCron.name}" executed. ${summaryText}`,
        type: "success",
      });
      setDetailTab("logs");
    } catch (err) {
