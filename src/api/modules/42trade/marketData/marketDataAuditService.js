...
function buildAuditMarketDataResult({
  symbolNorm,
  tfNorm,
  filePath,
  rawBars,
...
function auditMarketData(symbol, tf, options = {}) {
  ...
  const lowerBars =
    sourceTf && sourcePath
      ? barsStorage.readBarsFile(sourcePath, sourceTf, 0, {
          ...options,
          fullFile: true,
        })
      : [];

  return buildAuditMarketDataResult({
    symbolNorm,
    tfNorm,
    filePath,
    rawBars,
...
async function auditMarketDataAsync(symbol, tf, options = {}) {
  const symbolNorm = String(symbol || "").trim().toUpperCase();
  const tfNorm = barsStorage.normalizeCsvTfKey(tf);
  if (!symbolNorm || !tfNorm) {
    throw new Error("symbol and tf are required");
  ...
  const rawBars =
    filePath && String(filePath).trim()
      ? await barsStorage.readBarsFileAsync(filePath, tfNorm, 0, {
          ...options,
          fullFile: true,
        })
      : [];
  ...
  const lowerBars =
    sourceTf && sourcePath
      ? await barsStorage.readBarsFileAsync(sourcePath, sourceTf, 0, {
          ...options,
          fullFile: true,
        })
      : [];
...
function inspectAndRepairMarketData(symbol, tf, options = {}) {
  ...
  if (action === "rebuild" || action === "auto_fix") {
    out.operations.push({
      type: "rebuild_disabled",
      applied: false,
      reason: "derived_timeframe_rebuild_removed",
    });
  }
