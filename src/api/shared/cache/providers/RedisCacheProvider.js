...
class RedisCacheProvider {
  constructor({ url = DEFAULT_REDIS_URL, createClient: createRedisClient = createClient } = {}) {
    this.id = CACHE_PROVIDER_IDS.redis;
    this.url = String(url || "").trim() || DEFAULT_REDIS_URL;
    this.createClient = createRedisClient;
    this.client = null;
  ...
  async set(key, value, { ttlMs } = {}) {
    ...
    if (safeTtlMs > 0 && typeof client.pSetEx === "function") {
      await client.pSetEx(cacheKey, safeTtlMs, serialized);
      return true;
    }
    if (safeTtlMs > 0) {
      await client.set(cacheKey, serialized, { PX: safeTtlMs });
      return true;
    }
