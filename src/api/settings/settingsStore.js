...
} = require("../objects/objectLogService");
...
async function readNotificationConfigMap(userId) {
  const mergedPath = settingFilePath(
    userId,
    "notification_config",
    "preferences",
  );
  try {
    const raw = await readJsonFile(mergedPath);
  ...
  const legacyDir = notificationConfigDirForUser(userId);
  let entries = [];
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.name === "Notification_config.json") continue;
    const eventName = entry.name.replace(/\.json$/i, "");
    if (!eventName) continue;
    try {
      const row = await readSettingFile(path.join(legacyDir, entry.name));
      if (row?.data && typeof row.data === "object") {
        merged[eventName] = row.data;
      }
    } catch (err) {
      console.warn(
        `[settings-store] skipped invalid notification_config file ${path.join(legacyDir, entry.name)}:`,
        err.message,
      );
    }
  }
  ...
  if (Object.keys(merged).length > 0) {
    ...
    for (const entry of entries) {
      if (!entry.isFile() || !entry.name.endsWith(".json")) continue;
      if (entry.name === "Notification_config.json") continue;
      await fsp.unlink(path.join(legacyDir, entry.name)).catch((err) => {
        if (!err || err.code !== "ENOENT") throw err;
      });
    }
...
async function listLegacySettingsRows(userId, type = null) {
  const users = userId ? [String(userId)] : await listUsersWithSettings();
  const rows = [];
  for (const uid of users) {
    const scanRoot =
      type !== null &&
      ["settings", "system_config", "execution_profile"].includes(
        String(type || "")
          .trim()
          .toLowerCase(),
      );
    ...
    for (const file of files) {
      try {
        ...
        if (row.type === "notification_config") continue;
        if (type && row.type !== type) continue;
        if (!type && isProviderObjectSetting(row.type, row.name)) continue;
        if (!type && isCronObjectSetting(row.type)) continue;
        rows.push(row);
      } catch (err) {
        console.warn(
          `[settings-store] skipped invalid settings file ${file}:`,
          err.message,
        );
      }
    }
...
async function listUserSettingsByType(userId, type = null) {
  ...
  if (
    ...
    const legacyApiKeyRows = (
      await listLegacySettingsRows(userId, "api_key")
    ).filter((row) => !isProviderObjectSetting(row.type, row.name));
    return [
      ...rows.map(coerceProviderObjectRowToSettingRow),
      ...legacyApiKeyRows,
    ].sort((a, b) =>
      `${a.user_id}:${a.type}:${a.name}`.localeCompare(
        `${b.user_id}:${b.type}:${b.name}`,
  ...
  const rows = await listLegacySettingsRows(userId, type);
...
async function getUserSetting(userId, type, name = "default") {
  if (
    isRootFlatSetting(type, name) ||
    isNotificationConfigSetting(type) ||
    isExecutionProfileRootSetting(type, name)
  ) {
    await ensureFlattenedSettingMigration(userId, type, name);
  }
  ...
  if (isCronObjectSetting(type)) {
    const row = await objectStore.getObjectWithLegacyFallback(
      userId,
      "cron",
      name || "default",
    );
...
async function upsertUserSetting(
  ...
  if (isNotificationConfigSetting(type)) {
    const filePath = settingFilePath(userId, type, "preferences");
    ...
    return queueForFile(filePath, async () => {
      ...
      await ensureFlattenedSettingMigration(userId, type, eventKey);
      return [
  ...
  return queueForFile(filePath, async () => {
    ...
    await writeJsonAtomic(filePath, row.data);
    if (
      isRootFlatSetting(type, name) ||
      isExecutionProfileRootSetting(type, name)
    ) {
...
async function deleteUserSetting(userId, type, name = "default") {
  if (isNotificationConfigSetting(type)) {
    ...
    return queueForFile(filePath, async () => {
      ...
      await ensureFlattenedSettingMigration(userId, type, key);
    });
  }
