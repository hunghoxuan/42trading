...
function buildDerivedItemsFromBars(bars = [], timeframe = "") {
  return limitArtifactsNearLastBarByType(
    dedupeItems([
    ...buildSwingLevelItems(bars, timeframe),
    ...buildPreviousDayLevelItems(bars, timeframe),
    ...buildSupportDemandLevelItems(bars, timeframe),
    ...buildLiquidityItems(bars, timeframe),
    ...buildFvgZoneItems(bars, timeframe),
    ...buildOrderBlockItems(bars, timeframe),
    ...buildCandlePatternItems(bars, timeframe),
    ]),
    bars,
  );
}
...
function migrateLegacyMarketArtifacts(symbol, timeframe, options = {}) {
  const metadata = readJson(resolveMarketMetadataPath(symbol, timeframe, options), null);
...
function readTradeArtifacts({ tradeDir, tradeSid = "", symbol = "", timeframe = "" } = {}) {
  const filePath = resolveTradeArtifactPath(tradeDir);
  const envelope = readJson(filePath, null);
  if (envelope && typeof envelope === "object") return envelope;

  const legacyItems = readJson(resolveLegacyTradeObjectsPath(tradeDir), []);
