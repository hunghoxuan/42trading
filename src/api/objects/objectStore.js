...
function userRootDir(userId) {
  return path.join(DATA_ROOT, safePathPart(userId));
}

function objectTypeDir(userId, objectType) {
  return path.join(userRootDir(userId), canonicalObjectType(objectType));
}

function objectDir(userId, objectType, objectId) {
  return path.join(
...
function objectDataPath(userId, objectType, objectId) {
  return path.join(objectDir(userId, objectType, objectId), OBJECT_DATA_FILE);
}

function objectLogsDir(userId, objectType, objectId) {
  return path.join(
...
function legacySettingsDirForUser(userId) {
  return path.join(userRootDir(userId), "settings");
}

function legacySettingFilePath(userId, objectType, objectId) {
  const legacyType = legacySettingsTypeForObjectType(objectType);
  if (!legacyType) return null;
  return path.join(
    legacySettingsDirForUser(userId),
...
function normalizeObjectRow(row = {}, fallback = {}) {
  ...
  const requestedName = fallback.name || fallback.object_id || "default";
  const userId =
  ...
  const name =
    String(row.name || row.object_id || requestedName).trim() || "default";
...
async function listUsersWithObjectType(objectType) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const dir = path.join(DATA_ROOT, entry.name, objectTypeName);
...
async function listObjectsByType(userId, objectType) {
  const canonicalType = canonicalObjectType(objectType);
  const users = userId
    ? [String(userId)]
    : await listUsersWithObjectType(canonicalType);
  const rows = [];

  for (const uid of users) {
    const files = await listJsonObjectFolders(
      objectTypeDir(uid, canonicalType),
    );
...
async function listObjectsByTypeWithLegacyFallback(userId, objectType) {
  const rows = await listObjectsByType(userId, objectType);
  ...
  const users = userId
    ? [String(userId)]
    : await listUsersWithLegacySettings(legacyType);
  for (const uid of users) {
    const legacyDir = path.join(
      legacySettingsDirForUser(uid),
      safePathPart(legacyType, "settings"),
    );
    let entries;
    try {
...
async function listUsersWithLegacySettings(type) {
  let entries;
  try {
    entries = await fsp.readdir(DATA_ROOT, { withFileTypes: true });
  } catch (err) {
...
async function getObject(userId, objectType, objectId = "default") {
  const filePath = objectDataPath(userId, objectType, objectId || "default");
  try {
...
async function getObjectWithLegacyFallback(
  ...
  const primary = await getObject(userId, objectType, objectId);
  if (primary) return primary;

  const legacyPath = legacySettingFilePath(userId, objectType, objectId);
...
async function getObjectData(userId, objectType, objectId = "default") {
  const row = await getObject(userId, objectType, objectId);
...
async function getObjectDataWithLegacyFallback(
  ...
  const row = await getObjectWithLegacyFallback(userId, objectType, objectId);
  return row ? row.data : null;
}
...
async function upsertObject(
  ...
  const canonicalType = canonicalObjectType(objectType);
  const filePath = objectDataPath(userId, canonicalType, objectId || "default");
  return queueForFile(filePath, async () => {
    ...
    await ensurePrivateDir(
      objectLogsDir(userId, canonicalType, objectId || "default"),
    );
...
async function putObjectRow(row) {
...
async function deleteObject(userId, objectType, objectId = "default") {
  const canonicalType = canonicalObjectType(objectType);
  const dirPath = objectDir(userId, canonicalType, objectId || "default");
  return queueForFile(
    objectDataPath(userId, canonicalType, objectId || "default"),
    async () => {
      await fsp.rm(dirPath, { recursive: true, force: true });
      await pruneEmptyDirChain([
        objectTypeDir(userId, canonicalType),
        userRootDir(userId),
      ]);
    },
  );
