...
const syncGuards = require("../utils/syncGuards");
...
function createSqliteRepository(options = {}) {
  ...
  return {
    async upsertUser(user = {}) {
      ...
      db.prepare(
        `
        INSERT INTO users (user_id, name, email, role, metadata, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    async seedTrades(userId, rows = []) {
      ...
      const insert = db.prepare(`
        INSERT OR REPLACE INTO trades (
          sid, account_id, user_id, signal_id, source_id, strategy, entry_model,
          signal_tf, chart_tf, symbol, action, order_type, volume, entry, sl, tp,
      ...
      db.transaction(() => {
        db.prepare(
          ...
          ON CONFLICT(user_id) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        `,
        ).run(userIdSafe, userIdSafe, null, "USER", null, now, now);
    ...
    async brokerSyncTrades(userId, accountId, items = [], options = {}) {
      ...
      if (options.snapshotComplete) {
        ...
        for (const row of toClose) {
          ...
          closedRows.push(
            normalizeTradeRow(db.prepare(`SELECT * FROM trades WHERE sid = ?`).get(row.sid)),
          );
        }
      }

      const latestTrades = db
        .prepare(`SELECT * FROM trades WHERE user_id = ? AND account_id = ?`)
        .all(safeUserId(userId), aid)
        .map(normalizeTradeRow);
      const latestBySid = new Map(latestTrades.map((row) => [row.sid, row]));
      const tradeUpdates = [];
      for (const it of Array.isArray(items) ? items : []) {
...
function createPostgresRepository(options = {}) {
  ...
  return {
    ...
    async brokerSyncTrades(userId, accountId, items = [], options = {}) {
      ...
      let closedRows = [];
      if (options.snapshotComplete) {
        const latestAccountRes = await pool.query(
          `SELECT * FROM trades WHERE user_id = $1 AND account_id = $2`,
          [uid, aid],
        );
        const accountTrades = (latestAccountRes.rows || []).map(normalizeTradeRow);
        const toClose = accountTrades.filter((row) => {
          const exec = String(row.executionStatus || "").toUpperCase();
          if (!["FILLED", "PENDING"].includes(exec)) return false;
          if (seenTickets.size > 0) {
        ...
        for (const row of toClose) {
          ...
          closedRows.push(normalizeTradeRow(closeRes.rows?.[0] || null));
        }
      }
      ...
      const latestTrades = (latestRes.rows || []).map(normalizeTradeRow);
      const latestBySid = new Map(latestTrades.map((row) => [row.sid, row]));
      const tradeUpdates = [];
      for (const it of Array.isArray(items) ? items : []) {
        const sid = String(it.sid || "").trim();
