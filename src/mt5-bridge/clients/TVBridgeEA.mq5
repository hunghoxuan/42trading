...
input string InpLogFilter           = "Error,Reject"; // Comma-separated local log filter; blank = show all
...
string NormalizeLogToken(string value)
{
   string out = value;
   StringTrimLeft(out);
   StringTrimRight(out);
...
bool IsErrorStatus(const string status)
{
   string upper = status;
   StringToUpper(upper);
   return (upper == "FAIL" || upper == "ERROR" || upper == "REJECTED");
}

bool ShouldPrintLog(string msg, string level = "INFO")
{
   string filter = InpLogFilter;
   StringTrimLeft(filter);
   StringTrimRight(filter);
   if(filter == "")
      return true;

   string haystack = NormalizeLogToken(level) + " " + NormalizeLogToken(msg);
   string parts[];
   int count = StringSplit(filter, ',', parts);
   if(count <= 0)
   {
      string token = NormalizeLogToken(filter);
      return (token == "" || StringFind(haystack, token) >= 0);
   }

   for(int i = 0; i < count; i++)
   {
      string token = NormalizeLogToken(parts[i]);
      if(token == "")
         continue;
      if(StringFind(haystack, token) >= 0)
         return true;
   }
   return false;
}

void LocalLog(string msg, string level = "INFO")
{
   if(ShouldPrintLog(msg, level))
      Print(msg);
}
...
void ProcessAckQueue()
{
   ...
   for(int i=0; i<n; i++)
   {
      ...
      string url = BuildApiUrl("/api/broker/ack");
      ...
      {
         RemoteLog("[Ack] " + g_ackQStatus[i] + " sent for " + signalId,
                   IsErrorStatus(g_ackQStatus[i]) ? "ERROR" : "INFO");
         continue;
      }
            ...
            if(slDist < InpMinStopPips)
            {
               ...
               if(sltpMode == "ADJUST")
               {
                  ...
                  if(newVol < minVol)
                  {
                     ...
                     RemoteLog("[Reject] " + rejectMsg + " for " + signalId, "ERROR");
...
{
   string url = BuildApiUrl("/api/broker/tracked-symbols?account=" + IntegerToString((int)AccountInfoInteger(ACCOUNT_LOGIN)));
   string resp;
   if(!HttpGet(url, resp))
   {
...
void PushPrices()
{
   ...
   string url = BuildApiUrl("/api/broker/prices");
   string resp;
   if(HttpPostJsonWithResponse(url, body, resp))
   {
      g_pricePushCount++;
...
void PushBars()
{
   ...
   HttpPostJson(BuildApiUrl("/api/broker/bars"), body);
}
...
void SyncBarsIncremental()
{
   ...
   string covUrl = BuildApiUrl("/api/broker/symbols");
   string covResp;
   ...
   string resp;
   if(HttpPostJsonWithResponse(BuildApiUrl("/api/broker/prices-sync"), body, resp))
   {
