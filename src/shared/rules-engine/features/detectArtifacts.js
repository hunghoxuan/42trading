...
function inferPatternAt(bars = [], index = 0) {
  ...
  const upperShadowReversalShape =
    barStats.bodyRatio <= 0.35 &&
  ...
  if (
    prevStats.bearish &&
    barStats.bullish &&
    barStats.open < prevStats.close &&
    barStats.close > (prevStats.open + prevStats.close) * 0.5 &&
    barStats.close < prevStats.open &&
  ...
  if (
    prev2Stats &&
    prev2Stats.bearish &&
    prev2Stats.bodyRatio >= 0.45 &&
    prevStats.body <= prev2Stats.body * 0.6 &&
    prevStats.bodyRatio <= 0.35 &&
    barStats.bullish &&
    barStats.bodyRatio >= 0.45 &&
    barStats.close >= prev2Stats.bodyLow + prev2Stats.body * 0.5 &&
    preStarPressure === "down"
  ) {
  ...
  if (
    ...
    const opensWithinBodies =
      a && b && c &&
      b.open >= a.bodyLow && b.open <= a.bodyHigh &&
      c.open >= b.bodyLow && c.open <= b.bodyHigh;
