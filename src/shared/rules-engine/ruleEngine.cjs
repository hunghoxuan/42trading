...
function inferPreviousRuleNode(node) {
  ...
  if (rawValue.startsWith("indicators.")) return { var: rawValue.replace(/^indicators\./, "prev_indicators.") };
  if (rawValue.startsWith("bar.")) return { var: rawValue.replace(/^bar\./, "prev.") };
  return node;
}
...
function resolveCrossValues(rawValue, ctx, evaluate) {
  if (Array.isArray(rawValue)) {
    if (rawValue.length >= 4) {
      ...
      return [evaluate(leftPrevNode, ctx), evaluate(leftNode, ctx), evaluate(rightPrevNode, ctx), evaluate(rightNode, ctx)];
    }
  ...
  if (rawValue && typeof rawValue === "object") {
    ...
    return [evaluate(leftPrevNode, ctx), evaluate(rawValue.left, ctx), evaluate(rightPrevNode, ctx), evaluate(rawValue.right, ctx)];
  }
...
function evaluateComparatorTouches(rawValue, ctx, evaluate) {
  ...
  return Number.isFinite(level) && Number.isFinite(barHigh) && Number.isFinite(barLow)
...
function evaluateComparatorRetest(rawValue, ctx, evaluate) {
  ...
  const barHigh = Number(ctx?.bar?.high);
  const barLow = Number(ctx?.bar?.low);
  if (!Number.isFinite(level) || !Number.isFinite(prevValue) || !Number.isFinite(currentValue) || !Number.isFinite(barHigh) || !Number.isFinite(barLow)) return false;
  if (!(barLow <= level && barHigh >= level)) return false;
...
function evaluateComparatorRejected(rawValue, ctx, evaluate) {
  ...
  const barHigh = Number(ctx?.bar?.high);
  const barLow = Number(ctx?.bar?.low);
  if (!Number.isFinite(level) || !Number.isFinite(currentValue) || !Number.isFinite(barHigh) || !Number.isFinite(barLow)) return false;
...
const RULE_FUNCTION_NAMES = [
  "touches", "retest", "rejected", "holds_above", "holds_below", "sweeps_above", "sweeps_below",
  "sweep", "has_sweep", "bos", "has_bos", "choch", "has_choch", "breakout", "pin_bar", "engulfing",
  "inside_bar", "outside_bar", "reversal", "trend", "bias", "phase", "price_action_sl", "price_action_tp",
  "suggested_trade_sl", "suggested_trade_tp", "get_artifacts", "is_true", "draw",
];

const RULE_FUNCTION_EVALUATORS = Object.fromEntries(
  RULE_FUNCTION_NAMES.map((name) => [name, (args, ctx, evaluate) => strategyEventFunctions.evaluateNamedFunction(name, args, ctx, evaluate)]),
);
...
const RULE_OPERATOR_EVALUATORS = {
  ...
  crosses_above: (_values, rawValue, ctx, evaluate) => {
    const [leftPrev, leftCurrent, rightPrev, rightCurrent] = resolveCrossValues(rawValue, ctx, evaluate);
    return Number.isFinite(Number(leftPrev)) && Number.isFinite(Number(leftCurrent)) && Number.isFinite(Number(rightPrev)) && Number.isFinite(Number(rightCurrent)) &&
      Number(leftPrev) <= Number(rightPrev) && Number(leftCurrent) > Number(rightCurrent);
  },
...
function evaluateRuleExpression(node, ctx = {}) {
  ...
  if (Array.isArray(node)) return node.map((item) => evaluateRuleExpression(item, ctx));
  ...
  if (typeof node.fn === "string") {
    ...
    return evaluator(Array.isArray(node.args) ? node.args : [], ctx, evaluateRuleExpression);
  }
  const entries = Object.entries(node);
...
function normalizeRuleEvent({ rule = {}, result = true, ctx = {}, index = 0 } = {}) {
  ...
  const latest = strategyEventFunctions.isArtifactResult(result) && result.latest ? result.latest : null;
  ...
  const bias = String(rule?.outputs?.bias || latest?.subtype || latest?.direction || result?.meta?.bias || "").trim();
  return {
    ...
    evidence: {
      ...
      result: strategyEventFunctions.isArtifactResult(result)
        ? { functionName: result.functionName, latest, match_count: Array.isArray(result.matches) ? result.matches.length : 0, meta: result.meta || {} }
        : result,
    },
    artifact: { type: "marker", icon: rule.icon, label: rule.abbr },
  };
}
...
function evaluateRules({ bars = [], rules = [], baseContext = {}, buildContext = null } = {}) {
  ...
  normalizedBars.forEach((bar, index) => {
    const ctx = typeof buildContext === "function"
      ? buildContext({ bar, index, bars: normalizedBars, baseContext })
      : { ...baseContext, bars: normalizedBars, index, bar, prev: index > 0 ? normalizedBars[index - 1] : null };
