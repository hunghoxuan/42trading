...
// src/shared/strategy-engine/index.js
var strategy_engine_exports = {};
__export(strategy_engine_exports, {
  StrategyEngine: () => StrategyEngine,
  buildStrategySignal: () => buildStrategySignal,
  evaluateEventLogic: () => evaluateEventLogic,
  evaluateStrategies: () => evaluateStrategies,
  normalizeStrategyDefinition: () => normalizeStrategyDefinition
});
module.exports = __toCommonJS(strategy_engine_exports);

// src/shared/strategy-engine/strategyEngine.js
...
function filterEventsForNode(events = [], node = {}) {
  const matched = (Array.isArray(events) ? events : []).filter(
    (event) => eventMatchesNode(event, node)
  );
  ...
  const latestIndex = Math.max(
    ...matched.map((event) => eventBarIndex(event)).filter(Number.isFinite)
  );
...
function evaluateThen(children = []) {
  ...
  for (const group of eventGroups) {
    const candidate = group.slice().sort((left, right) => {
...
function normalizeStrategyDefinition(strategy = {}, index = 0) {
  ...
  return {
    ...
    event_logic: strategy?.event_logic && typeof strategy.event_logic === "object" ? strategy.event_logic : strategy?.when && typeof strategy.when === "object" ? strategy.when : null,
    actions: Array.isArray(strategy?.actions) ? strategy.actions : [],
    metadata: strategy?.metadata && typeof strategy.metadata === "object" && !Array.isArray(strategy.metadata) ? { ...strategy.metadata } : {}
  };
}
function buildStrategySignal({ strategy = {}, result = {}, events = [] } = {}) {
  const latest = (Array.isArray(result.events) ? result.events : []).slice().sort((left, right) => (eventTime(right) ?? 0) - (eventTime(left) ?? 0))[0] || null;
  return {
...
function evaluateStrategies({ events = [], strategies = [] } = {}) {
  const normalizedStrategies = (Array.isArray(strategies) ? strategies : []).map(normalizeStrategyDefinition).filter((strategy) => strategy.event_logic);
  const signals = [];
