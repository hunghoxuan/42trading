...
function eventMatchesNode(event = {}, node = {}) {
  ...
  if (id && normalizeEventId(event.rule_id) !== id && normalizeEventId(event.event_id) !== id) {
    return false;
  }
...
function filterEventsForNode(events = [], node = {}) {
  ...
  if (!Number.isFinite(withinBars) || withinBars <= 0 || matched.length <= 1) {
    return matched;
  }
  const latestIndex = Math.max(
    ...matched
      .map((event) => eventBarIndex(event))
...
function combineResult(kind = "and", children = []) {
  ...
  if (kind === "and" && normalized.some((child) => !child.matched)) {
    return { matched: false, events: [] };
  }
  if (kind === "or" && normalized.every((child) => !child.matched)) {
    return { matched: false, events: [] };
  }
  return {
    matched: true,
    events: normalized.flatMap((child) => child.events || []),
  };
}
...
function evaluateThen(children = []) {
  ...
  for (const group of eventGroups) {
    const candidate = group
      .slice()
      .sort((left, right) => {
        const leftTime = eventTime(left) ?? 0;
        const rightTime = eventTime(right) ?? 0;
...
function evaluateEventLogic(node = {}, events = []) {
  ...
  if (Array.isArray(node.and)) {
    return combineResult("and", node.and.map((child) => evaluateEventLogic(child, events)));
  }
  if (Array.isArray(node.or)) {
    return combineResult("or", node.or.map((child) => evaluateEventLogic(child, events)));
  }
  if (Array.isArray(node.then)) {
    return evaluateThen(node.then.map((child) => evaluateEventLogic(child, events)));
  }
  if (node.not) {
...
function normalizeStrategyDefinition(strategy = {}, index = 0) {
  ...
  return {
    ...
    event_logic:
      strategy?.event_logic && typeof strategy.event_logic === "object"
...
class StrategyEngine {
  constructor({ strategies = [] } = {}) {
    this.strategies = (Array.isArray(strategies) ? strategies : []).map(
