...
function normalizeStrategyConditions(strategy = {}) {
  const raw =
    strategy?.conditions && typeof strategy.conditions === "object" && !Array.isArray(strategy.conditions)
      ...
      : strategy?.metadata?.conditions &&
        ...
        : {};
  const tradeConfig =
    strategy?.settings?.trade_config &&
    typeof strategy.settings.trade_config === "object" &&
    !Array.isArray(strategy.settings.trade_config)
      ? strategy.settings.trade_config
      : {};
  const presetList = (value) =>
    Array.isArray(value)
      ? value
      : String(value || "")
          .split(/[;,\s]+/)
  ...
  return {
    ...
    symbols: normalizeStringList(
      presetList(tradeConfig.symbols).length
        ? presetList(tradeConfig.symbols)
        : raw?.symbols,
    ),
    skip_news:
      String(tradeConfig.news_block || "").trim()
        ? String(tradeConfig.news_block).trim().toLowerCase() !== "no"
        : raw?.skip_news === true || raw?.avoid_news === true,
