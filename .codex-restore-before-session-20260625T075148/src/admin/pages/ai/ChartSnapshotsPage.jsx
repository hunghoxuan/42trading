...
function aiSourceFromModel(modelRaw) {
  ...
  if (
    ...
    model.includes("2.5vl") ||
    model.includes("visio") ||
...
export default function ChartSnapshotsPage() {
  ...
  const [browserTfs, setBrowserTfs] = useState(["4h", "15m"]);
  const [analyzeHeaderPreset, setAnalyzeHeaderPreset] = useState("4h|15m");
  const [analyzeTfMenuOpen, setAnalyzeTfMenuOpen] = useState(false);
  const analyzeTfMenuRef = useRef(null);
  const analyseSettingsReadyRef = useRef(false);
  const lastSavedAnalyseSettingsRef = useRef("");
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        if (s?.data && typeof s.data === "object") {
          ...
          if (savedBrowserTfs.length > 0) {
            ...
            setAnalyzeHeaderPreset("4h|15m");
          }
          lastSavedAnalyseSettingsRef.current = JSON.stringify({
            lookbackBars: s.data.lookbackBars,
            snapshotQuality: s.data.snapshotQuality,
            mergeSnapshots: s.data.mergeSnapshots,
            broker: s.data.broker || "",
            refreshSnapshot: s.data.refreshSnapshot !== false,
            browserTfs: Array.isArray(s.data.browserTfs)
              ? s.data.browserTfs
              : [],
          });
        }
      ...
      .catch(() => {})
      .finally(() => {
        analyseSettingsReadyRef.current = true;
      });
  }, []);
  ...
  const saveSettings = useCallback(() => {
    const payload = buildAnalyseSettingsPayload();
    return api
      .upsertSetting({
        type: "settings",
      ...
      .then(() => {
        lastSavedAnalyseSettingsRef.current = JSON.stringify(payload);
  ...
  const lastResponseErrorToastRef = useRef({
    key: "",
    at: 0,
  });
  ...
  const [aiModelConfig, setAiModelConfig] = useState(() => ({
    providers: {
      ...
      ai_codex: {
        label: "Local Codex",
        models: [{ value: "codex-42trade", label: "Codex 42Trade" }],
      },
      ai_deepseek: {
        label: "DeepSeek",
      ...
      ai_ollama: {
        ...
        models: [
          { value: "qwen2.5vl:3b", label: "Qwen2.5 VL 3B" },
          { value: "llava:latest", label: "LLaVA 1.5" },
        ],
      },
    },
  }));
  ...
  const [watchlistPendingSymbols, setWatchlistPendingSymbols] = useState([]);
  const [customSymbolGroups, setCustomSymbolGroups] = useState([]);
  const [isSymbolPanelOpen, setIsSymbolPanelOpen] = useState(true);
  const [isAnalyzePanelOpen, setIsAnalyzePanelOpen] = useState(true);
  ...
  const [tradeSymbolsLoadingCount, setTradeSymbolsLoadingCount] = useState(0);
  const tradeSymbolsLoading = tradeSymbolsLoadingCount > 0;
  const tradeSymbolsLoadedRef = useRef({
    pending: false,
    filled: false,
  });
  const tradeSymbolsInflightRef = useRef({
    pending: null,
    filled: null,
  });
  ...
  const showResponseLoadErrorToast = useCallback(
    (message, sid = "") => {
      const key = `${String(sid || "").trim()}::${String(message || "").trim()}`;
      const now = Date.now();
      if (
        lastResponseErrorToastRef.current.key === key &&
        now - Number(lastResponseErrorToastRef.current.at || 0) < 2000
      ) {
        return;
      }
      lastResponseErrorToastRef.current = { key, at: now };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const res = await fetch(
          `${base}/api/trades/${encodeURIComponent(tradeSidFromRoute)}/response`,
        );
        if (!res.ok) {
          let message = `Failed to load response (${res.status})`;
          try {
            const err = await res.json();
            message = String(
              err?.analysis_error?.error || err?.error || message,
            ).trim() || message;
          } catch {}
          setResponseLoadError(message);
          setAnalysisParsed(null);
          setAnalysisRaw(`Analysis failed: ${message}`);
          setAnalysisJson(
            JSON.stringify(
              { ok: false, trade_sid: tradeSidFromRoute, error: message },
              null,
              2,
            ),
          );
          showResponseLoadErrorToast(message, tradeSidFromRoute);
          return;
        }
        const data = await res.json();
        if (!data?.ok && !data?.parsed_json) {
          const message = String(
            data?.analysis_error?.error ||
              data?.error ||
              "Saved analysis response is not ready",
          ).trim();
          setResponseLoadError(message);
          setAnalysisParsed(null);
          setAnalysisRaw(`Analysis failed: ${message}`);
          ...
          showResponseLoadErrorToast(
            message,
            String(data?.sid || data?.session_id || tradeSidFromRoute || ""),
          );
          return;
        }
        if (data.parsed_json) {
        ...
        const message = String(
          error?.message || "Failed to load saved analysis response",
        );
        setResponseLoadError(message);
        setAnalysisParsed(null);
        setAnalysisRaw(`Analysis failed: ${message}`);
    ...
    load();
  }, [tradeSidFromRoute, showResponseLoadErrorToast]);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        setTradeDetailPlan(extractedPlan);
        ...
        setPosition((prev) => ({
          ...
          direction: String(
            extractedPlan.direction || exact.action || exact.side || "BUY",
          )
            .trim()
            .toUpperCase(),
          entry:
            extractedPlan.entry != null ? String(extractedPlan.entry) : "",
          tp: extractedPlan.tp != null ? String(extractedPlan.tp) : "",
          tp2: extractedPlan.tp2 != null ? String(extractedPlan.tp2) : "",
          tp3: extractedPlan.tp3 != null ? String(extractedPlan.tp3) : "",
          sl: extractedPlan.sl != null ? String(extractedPlan.sl) : "",
          rr:
            extractedPlan.rr != null
              ? String(extractedPlan.rr)
              : exact.rr_planned != null
                ...
                : exact.rr != null
                  ...
                  : "",
          trade_type: String(
            extractedPlan.trade_type ||
              exact.order_type ||
              exact.type ||
              "limit",
          ).toLowerCase(),
          note: String(extractedPlan.note || exact.note || ""),
  ...
  const chartUiConfigKey = useMemo(
    () =>
      `${String(provider || "").toUpperCase()}|${String(cfg.lookbackBars || "1000")}|${(Array.isArray(browserTfs) ? browserTfs : []).join(",")}`,
  ...
  const hydrateFromResultEntry = useCallback(
    (entry) => {
      ...
      const directParsed =
        out?.parsed_json && typeof out.parsed_json === "object"
          ? out.parsed_json
          : null;
      const parsed = enrichParsedAnalysis(
        raw,
        directParsed || tryParseJsonLoose(raw),
      );
      ...
      pendingHydrateRef.current = {
        raw: raw || (directParsed ? JSON.stringify(directParsed, null, 2) : ""),
        parsed: parsed && typeof parsed === "object" ? parsed : null,
        usedFiles: used,
  ...
  const responseText = useMemo(
    () =>
      responseLoadError
        ? `Failed: ${responseLoadError}`
        : buildFriendlyResponse(effectiveParsed),
    [effectiveParsed, responseLoadError],
  );
  ...
  const saveAnalyseSettingsSilent = useCallback((data = {}) => {
    return api
      ...
      .then(() => {
        lastSavedAnalyseSettingsRef.current = JSON.stringify(data || {});
      });
  }, []);
  ...
  const fetchAllBars = async (symbol, tfs, bars) => {
    ...
    for (const tf of tfs) {
      try {
        ...
        const out = await api.chartCandles(sym, tf, bars, true);
  ...
  const fetchBarsSnapshot = async (symbol, tf, bars, forceRefresh = false) => {
    ...
    try {
      const { promise: barsPromise } = NotificationHub.track(
        ...
        () => api.chartCandles(sym, tf, bars, forceRefresh),
      );
  ...
  const analyzeFiles = async (files = [], opts = {}) => {
    ...
    try {
      ...
      if (out?.auto_save_result?.saved) {
        const autoEntity = resolveCreatedId(out?.auto_save_result, "trade");
        if (autoEntity)
      ...
      setAnalysisRaw(raw || (directParsed ? JSON.stringify(directParsed, null, 2) : ""));
      const rawParsed = raw ? tryParseJsonLoose(raw) : null;
      // Extract canonical parsed_json from API response wrapper when present
      const canonicalPayload =
        directParsed && !Array.isArray(directParsed)
          ? directParsed
  ...
  const loadTradeSymbols = useCallback((status, { force = false } = {}) => {
    const statusKey = status === "PENDING" ? "pending" : "filled";
    if (!force && tradeSymbolsLoadedRef.current[statusKey]) {
      return Promise.resolve([]);
    }
    if (!force && tradeSymbolsInflightRef.current[statusKey]) {
      return tradeSymbolsInflightRef.current[statusKey];
    }
    setTradeSymbolsLoadingCount((prev) => prev + 1);
    ...
    const request = api
      ...
      .then((data) => {
        ...
        tradeSymbolsLoadedRef.current[statusKey] = true;
        return items;
      })
      .catch((err) => {
      ...
      .finally(() => {
        if (tradeSymbolsInflightRef.current[statusKey] === request) {
          tradeSymbolsInflightRef.current[statusKey] = null;
        }
        setTradeSymbolsLoadingCount((prev) => Math.max(0, prev - 1));
      });
    tradeSymbolsInflightRef.current[statusKey] = request;
    return request;
  }, []);
  ...
  const saveWatchlistToDb = async (nextList) => {
    try {
      ...
      const latestSymbolGroups = normalizeSymbolGroupsData(
        (Array.isArray(latestSettings?.settings) ? latestSettings.settings : [])
          .find(
            (item) =>
              item?.type === "symbol_groups" && item?.name === "default",
          )?.data || {},
      );
      const currentGroups = Array.isArray(latestSymbolGroups?.groups)
        ? latestSymbolGroups.groups
        : [];
      const hasWatchlistGroup = currentGroups.some(
      ...
      const result = await api.upsertSetting({
        type: "symbol_groups",
        name: "default",
        data: nextData,
      });
      const persistedData = normalizeSymbolGroupsData(
        result?.item?.data || nextData,
      );
      const persistedWatchlist = [
        ...new Set(
          (getWatchlistGroup(persistedData)?.symbols || [])
            .map(normalizeWatchSymbol)
            .filter(Boolean),
        ),
      ];
      setSymbolGroupsData(persistedData);
      setCustomSymbolGroups(getCustomSymbolGroups(persistedData));
  ...
  const setWatchlistSymbolPending = useCallback((symbol, pending) => {
    const normalized = normalizeWatchSymbol(symbol);
    if (!normalized) return;
    setWatchlistPendingSymbols((prev) => {
  ...
  const moveWatchlistSymbol = useCallback(
    (fromSymbol, toSymbol) => {
      ...
      setWatchlist(next);
      saveWatchlistToDb(next).catch((error) => {
        console.error("[watchlist] Reorder failed:", error?.message || error);
        loadWatchlist();
      });
    },
  ...
  const addCurrentSymbolToWatchlist = async () => {
    ...
    try {
      setWatchlistSymbolPending(s, true);
      const persistedWatchlist = await saveWatchlistToDb(next);
      setStatus({
        type: "success",
        text: `Added to watchlist: ${s} (${persistedWatchlist.length})`,
      });
    } catch (e) {
      setStatus({
        type: "error",
      ...
      setWatchlistSymbolPending(s, false);
    }
  };

  const removeFromWatchlist = async (s) => {
    const target = normalizeWatchSymbol(s);
    const next = (Array.isArray(watchlist) ? watchlist : []).filter(
      (x) => normalizeWatchSymbol(x) !== target,
    );
    try {
      setWatchlistSymbolPending(target, true);
      await saveWatchlistToDb(next);
      setStatus({ type: "success", text: `Removed from watchlist: ${target}` });
    } catch (e) {
      setStatus({
        type: "error",
      ...
      setWatchlistSymbolPending(target, false);
    }
  };
  ...
  useEffect(() => {
    const check = () => {
      ...
      if (isMobile) {
        setIsSymbolPanelOpen(false);
        setIsAnalyzePanelOpen(false);
      }
    };
    check();
  ...
  const shouldEagerLoadTradeSymbols =
    isTradeRoute ||
    isManualRoute ||
  ...
  useEffect(() => {
    if (!shouldEagerLoadTradeSymbols) return;
    loadTradeSymbols("PENDING");
    loadTradeSymbols("FILLED");
  }, [loadTradeSymbols, shouldEagerLoadTradeSymbols]);
  ...
  useEffect(() => {
    if (!analyseSettingsReadyRef.current) return undefined;
    if (autoSaveTimerRef.current) clearTimeout(autoSaveTimerRef.current);
    const nextPayload = buildAnalyseSettingsPayload();
    const nextSerialized = JSON.stringify(nextPayload);
    if (nextSerialized === lastSavedAnalyseSettingsRef.current) {
      return undefined;
    }
    autoSaveTimerRef.current = setTimeout(() => {
      saveAnalyseSettingsSilent(nextPayload).catch(() => {});
    }, 250);
    return () => {
      if (autoSaveTimerRef.current) clearTimeout(autoSaveTimerRef.current);
    };
      ...
      <div style={{ display: "grid", gap: 10 }}>
        ...
        >
          ...
          <div style={{ minWidth: 110 }}>
            <label className="minor-text">Bars</label>
            <FormComboSelect
          ...
          <div style={{ minWidth: 90 }}>
            <label className="minor-text">Quality</label>
            <FormComboSelect
          ...
          >
            <input
              ...
              onChange={(e) => setCfgField("mergeSnapshots", e.target.checked)}
          ...
          >
            <input
              ...
              onChange={(e) => setCfgField("refreshSnapshot", e.target.checked)}
              style={{ cursor: "pointer" }}
            />
            New
          </label>
    ...
    <>
      ...
      {settingsTab === "guide" ? (
        <>
          <GroupButtons
            items={[
              { label: "User Custom", value: "user" },
              { label: "System", value: "system" },
            ]}
            selectedItems={[guideSubTab]}
            selectionMode="single"
            onChange={([nextTab]) => setGuideSubTab(nextTab)}
            border_type="single"
            size="compact"
            buttonStyle={{ fontSize: 11, padding: "3px 10px" }}
            style={{ marginBottom: 12 }}
            ariaLabel="Guide sub tabs"
          />
          {guideSubTab === "user" ? (
            <>
        ...
        <>
          <GroupButtons
            items={[
              ...
              { label: "Trade Plan", value: "system" },
            ]}
            selectedItems={[schemaSubTab]}
            selectionMode="single"
            onChange={([nextTab]) => setSchemaSubTab(nextTab)}
            border_type="single"
            size="compact"
            buttonStyle={{ fontSize: 11, padding: "3px 10px" }}
            style={{ marginBottom: 12 }}
            ariaLabel="Schema sub tabs"
          />
  ...
  const watchlistPendingSet = useMemo(
    () =>
      new Set(
        (Array.isArray(watchlistPendingSymbols) ? watchlistPendingSymbols : [])
          .map(normalizeWatchSymbol)
          .filter(Boolean),
      ),
    [watchlistPendingSymbols],
  );
  ...
  const symbolFilterTabOptions = useMemo(
    () => [
      SYMBOL_FILTER_TAB_OPTIONS[0],
      ...customGroupTabOptions.map((group) => ({
        value: group.key,
        label: group.label,
      })),
      ...SYMBOL_FILTER_TAB_OPTIONS.slice(1),
    ],
    [customGroupTabOptions],
  );

  const selectedSymbolFilterTabLabel = useMemo(
    () =>
      symbolFilterTabOptions.find((option) => option.value === symbolFilterTab)
        ?.label || "Watchlist",
    [symbolFilterTab, symbolFilterTabOptions],
  );
  ...
  const activeTradeFilterSymbols = useMemo(() => {
    const chosen = selectedSymbols.length ? selectedSymbols : symbolsByTab;
    return new Set(
      (Array.isArray(chosen) ? chosen : [])
        .map((symbol) => normalizeWatchSymbol(symbol))
        .filter(Boolean),
    );
  }, [selectedSymbols, symbolsByTab]);

  const filteredTradeRowsByStatus = useMemo(() => {
    const filterRows = (rows = []) => {
      if (!activeTradeFilterSymbols.size) return Array.isArray(rows) ? rows : [];
      return (Array.isArray(rows) ? rows : []).filter((row) =>
        activeTradeFilterSymbols.has(
          normalizeWatchSymbol(String(row?.symbol || "")),
        ),
      );
    };
    return {
      filled: filterRows(tradeRowsByStatus.filled),
      pending: filterRows(tradeRowsByStatus.pending),
    };
  }, [activeTradeFilterSymbols, tradeRowsByStatus]);

  const showFilledTradeSection = symbolFilterTab !== "PENDING";
  const showPendingTradeSection = symbolFilterTab !== "FILLED";
  const showInlineAnalyzePanel = !hasAnalyzeResponse && isAnalyzeRoute;
  ...
  return (
    <section
      className="snapshot-builder-v2 snapshot-builder-v3 snapshot-builder-ai-v4"
      style={
        isResponseRoute
    ...
    >
      <ResponsivePanel
        headerContent={
          ...
          >
            {symbolFilterTabOptions.map((option) => (
        ...
        open={isSymbolPanelOpen}
        onOpenChange={setIsSymbolPanelOpen}
        collapseDirection="left-right"
        border="always"
        className="snapshot-col-v3 snapshot-col-symbols-v3"
        bodyClassName="snapshot-symbols-panel-body"
        style={
          isResponseRoute
            ? { display: "none" }
            : isSymbolPanelOpen
      ...
      >
        ...
        >
          ...
          <div
            className="snapshot-trade-groups-v4"
            style={{
              display: "flex",
              flexDirection: "column",
              overflow: "visible",
          ...
          >
            <>
                {showFilledTradeSection ? (
                  <div className="snapshot-trade-group-v4">
                    <div
                      className="minor-text"
                      style={{ padding: "2px 0", fontWeight: 700, fontSize: 11 }}
                    >
                      Filled ({filteredTradeRowsByStatus.filled.length})
                    </div>
                    <div
                      className="snapshot-activity-list-v4"
                      style={{ overflow: "visible", maxHeight: "none" }}
                    >
                      {filteredTradeRowsByStatus.filled.length === 0 ? (
                        <div className="minor-text">No filled trades.</div>
                      ) : (
                        filteredTradeRowsByStatus.filled.map((t) => {
                        const sideRaw = String(
                          t?.action || t?.side || "",
                        ).toUpperCase();
                ...
                {showPendingTradeSection ? (
                  <div className="snapshot-trade-group-v4">
                    <div
                      className="minor-text"
                      style={{ padding: "2px 0", fontWeight: 700, fontSize: 11 }}
                    >
                      Pending ({filteredTradeRowsByStatus.pending.length})
                    </div>
                    <div
                      className="snapshot-activity-list-v4"
                      style={{ overflow: "visible", maxHeight: "none" }}
                    >
                      {filteredTradeRowsByStatus.pending.length === 0 ? (
                        <div className="minor-text">No pending trades.</div>
                      ) : (
                        filteredTradeRowsByStatus.pending.map((t) => {
                        const sideRaw = String(
                          t?.action || t?.side || "",
                        ).toUpperCase();
                        const isBuy = sideRaw.includes("BUY");
      ...
      <ResponsivePanel
        showToggle={false}
        border={showInlineAnalyzePanel ? "none" : "always"}
        className="snapshot-col-v3 snapshot-col-settings-v3"
        style={isResponseRoute ? { gridColumn: "1 / -1" } : {}}
      >
        {showInlineAnalyzePanel ? null : topControls}

        {showInlineAnalyzePanel && (
          <ResponsivePanel
            headerContent={topControlsBar}
            open={isAnalyzePanelOpen}
            onOpenChange={setIsAnalyzePanelOpen}
            collapseDirection="top-down"
            border="always"
            className="snapshot-analyze-panel-v4 fadeIn"
            style={{ marginBottom: isAnalyzePanelOpen ? 12 : 0 }}
          >
            {isAnalyzePanelOpen ? (
              <div
        ...
        {!hasAnalyzeResponse && isAnalyzeRoute && selectedSymbols.length === 0 ? (
                              ...
                              {filteredSyms.map((sym) => (
                                ...
                                >
                                  <SymbolChart
                                    ...
                                    defaultMode={isAnalyzeRoute ? "live" : "cache"}
                                    initialGridCols={effectiveGridCols}
                                    initialBarsCount={Number(
                                      cfg.lookbackBars || 1000,
                                    )}
                      ...
                      return filtered.slice(0, visibleCount).map((sym) => (
                        ...
                        >
                          <SymbolChart
                            ...
                            defaultMode={isAnalyzeRoute ? "live" : "cache"}
                            initialGridCols={effectiveGridCols}
                            initialBarsCount={Number(cfg.lookbackBars || 1000)}
                            autoLoadOnMount={true}
                            showPerCardLayoutControls={false}
                            fillViewportForFourCharts={true}
                            analyzeLabel={
                            ...
                            onToggleWatchlist={(s) => {
                              ...
                              if (watchlistPendingSet.has(sn)) return;
                              ...
                              if (inWatchlist) {
                                ...
                                const next = [...new Set([...watchlist, s])];
                                setWatchlistSymbolPending(sn, true);
                                saveWatchlistToDb(next)
                                  .catch((error) => {
                                    setStatus({
                                      type: "error",
                                  ...
                                  .finally(() => {
                                    setWatchlistSymbolPending(sn, false);
                                  });
                              }
                            }}
                            onRemoveSelected={(s) => {
        ...
        {!isTradeRoute &&
          (selectedSymbol || (isResponseRoute && hasAnalyzeResponse)) &&
          (selectedSymbols.length <= 1 ||
            hasAnalyzeResponse ||
            isManualRoute) && (
            <Suspense
            ...
            >
              <SignalDetailCard
                key={selectedSymbol || `response:${analyzeSessionId || tradeSidFromRoute || "detail"}`}
                mode="ai"
          ...
          >
            ...
            >
              <GroupButtons
                ...
                selectedItems={[settingsTab]}
                selectionMode="single"
                onChange={([nextTab]) => setSettingsTab(nextTab)}
                border_type="single"
                className="snapshot-tabs-v2"
                buttonStyle={{ fontSize: 11, padding: "4px 10px" }}
                style={{ margin: 0 }}
                ariaLabel="Snapshot settings tabs"
              />
              <div
                style={{
                  display: "flex",
