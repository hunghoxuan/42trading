...
} from "../utils/symbolChartStreaming";
...
function applyStreamLifecycleState(topic, error = null) {
  ...
  if (errorType === "disconnect") {
    chartStreamStore.setDisconnected(topic, error?.message || error?.reason || "Realtime socket disconnected");
    return;
  }
  if (error) {
...
export function useSymbolChartData({
            ...
            try {
              ...
              if (tfData.bars.length > 0) {
                chartStreamStore.setBootstrap(
                  buildHistoryTrackingTopic(sym, tf, hasAnchoredEndTime ? endTimeSec : null),
    ...
    async (tf, opts = {}) => {
      ...
      try {
        if (mode === "cache") {
          ...
          const shouldUseSocketHistory = false;
            ...
            if (historyEndTimeSec) {
              ...
              try {
                ...
                logHistoryConsole("response", {
                  ...
                  returnedStartSec: Number(snap?.bar_start || snap?.bars?.[0]?.time || 0) || null,
                ...
                if (snap?.bars?.length) {
                  chartStreamStore.completeHistoryRequest(topic, requestKey, {
                    ...
                    range: {
                      startSec: Number(snap?.bar_start || snap?.bars?.[0]?.time || 0) || 0,
                      endSec:
                        Number(
                          snap?.bar_end ||
                  ...
                  if (exhausted) {
                    chartStreamStore.markHistoryExhausted(topic, requestRange, snap?.metadata || null);
                  }
          ...
          if (
            ...
            if (tfData.bars.length > 0) {
              logHistoryConsole("response", {
                ...
                returnedStartSec: Number(tfData?.bar_start || tfData?.bars?.[0]?.time || 0) || null,
                returnedStartIso: formatHistoryTimeSec(
                  Number(tfData?.bar_start || tfData?.bars?.[0]?.time || 0) || null,
                ),
                returnedEndSec:
                  Number(tfData?.bar_end || tfData?.bars?.[tfData?.bars?.length - 1]?.time || 0) ||
                  null,
                returnedEndIso: formatHistoryTimeSec(
                  Number(
          ...
          if (tfData.bars.length > 0) {
            ...
            if (isSocketHistoryRequest) {
              setData((prev) => {
                ...
                const currentCount = Array.isArray(current?.bars) ? current.bars.length : 0;
                const merged = mergeHistoricalBarsIntoTfData(current, {
                  bars: tfData.bars,
                  cachedAt: tfData.cached_at,
                  metadata: tfData.metadata,
                });
                const mergedCount = Array.isArray(merged?.bars) ? merged.bars.length : 0;
                logHistoryConsole("merge", {
                  symbol: sym,
                  timeframe: tfKey,
                  previousBars: currentCount,
                  incomingBars: Array.isArray(tfData.bars) ? tfData.bars.length : 0,
                  mergedBars: mergedCount,
                  addedBars: Math.max(0, mergedCount - currentCount),
                  mergedStartSec: Number(merged?.bars?.[0]?.time || 0) || null,
                  mergedStartIso: formatHistoryTimeSec(
                    Number(merged?.bars?.[0]?.time || 0) || null,
                  ),
                  mergedEndSec:
                    Number(merged?.bars?.[merged?.bars?.length - 1]?.time || 0) || null,
                  mergedEndIso: formatHistoryTimeSec(
                    Number(merged?.bars?.[merged?.bars?.length - 1]?.time || 0) || null,
                  ),
                });
  ...
  useEffect(() => {
    const hasAnchoredEndTime =
      Number.isFinite(Number(endTimeSec)) && Number(endTimeSec) > 0;
    if (!sym || skipFetch || mode !== "cache" || hasAnchoredEndTime || !tfs.length) {
      return undefined;
    }
    let cancelled = false;
    const unsubscribers = [];
  ...
  }, [barsCount, endTimeSec, mode, profile, skipFetch, sym, tfs]);
