...
| `CTraderSyncEngine` | Complete deterministic synchronization runtime: timer cadence, sync/ack/bar payload composition, acknowledgement classification and retry timing, event/result classification, incremental coverage parsing and fetch planning, Unix-time conversion, and closed-ticket retention | Copy this class with `CTraderSyncCoverage` and `CTraderIncrementalFetchPlan`; live cTrader snapshot extraction, queue action execution, HTTP dispatch, main-thread marshaling, logging, and panel state remain adapters in `TVBridgeCBot`/`CTraderTransportEngine` |
| `CTraderChartEngine` | Complete reusable chart runtime: reflection-safe cTrader styling, chart-object cleanup and budgets, blurred-candle and indicator-line primitives, phased redraw policy, marker/icon/font policy, HTF splitter and mini-chart layout, dashboard text formatting, artifact-label normalization/priority, and toolbar selection policy | Copy this class with `CTraderChartSplitterStyle`; live `Bars`/indicator/event collection, composite overlay orchestration, cTrader control construction/event wiring, risk-dashboard data gathering, and timer callbacks remain adapters in `TVBridgeCBot` so platform state and cache cadence stay unchanged |
