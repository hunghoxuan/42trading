const { pgTable, text, integer, doublePrecision, boolean, timestamp, jsonb, serial, uuid, uniqueIndex } = require("drizzle-orm/pg-core");
...
const users = pgTable("users", {
  ...
  isActive: boolean("is_active").default(true), metadata: jsonb("metadata"),
...
const userAccounts = pgTable("user_accounts", {
  ...
  sourceIdsCache: jsonb("source_ids_cache"), metadata: jsonb("metadata"), status: text("status"),
...
const userTemplates = pgTable("user_templates", {
  ...
  name: text("name").notNull(), data: jsonb("data").notNull(), status: text("status").default("ACTIVE"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow(),
});
...
const userSettings = pgTable("user_settings", {
  ...
  userId: text("user_id").notNull().references(() => users.userId, { onDelete: "cascade" }),
  name: text("name").notNull().default("default"), type: text("type").notNull(), data: jsonb("data").notNull(),
...
const signals = pgTable("signals", {
  ...
  rawJson: jsonb("raw_json"), status: text("status").default("NEW"), profile: text("profile"),
...
const trades = pgTable("trades", {
  ...
  confluenceChecklist: jsonb("confluence_checklist"), skipRecommendation: text("skip_recommendation"),
  riskManagement: jsonb("risk_management"), note: text("note"), leaseToken: text("lease_token"),
  ...
  pnlRealized: doublePrecision("pnl_realized"), metadata: jsonb("metadata"), rawJson: jsonb("raw_json"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow(),
});
...
const logs = pgTable("logs", {
  ...
  metadata: jsonb("metadata"), userId: text("user_id"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
...
const marketData = pgTable("market_data", {
  ...
  bars: jsonb("bars"), source: text("source"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
});
