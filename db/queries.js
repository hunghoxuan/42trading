...
function toSnake(r) {
  ...
  for (const [k, v] of Object.entries(r))
    o[k.replace(/[A-Z]/g, (m) => "_" + m.toLowerCase())] = v;
  return o;
}
function rowsSnake(rs) {
  return Array.isArray(rs) ? rs.map(toSnake) : rs;
}
...
async function listTradesV2(db, filters = {}, page = 1, pageSize = 50) {
  ...
  const items = await db
    ...
    .orderBy(
      desc(
        sql`COALESCE(${schema.trades.closedAt}, ${schema.trades.updatedAt})`,
      ),
...
async function upsertSignal(db, signal) {
  const v = {
    ...
    riskMoneyPlanned:
      signal.risk_money_planned != null
        ...
        : null,
    riskPctPlanned:
      signal.risk_pct_planned != null ? Number(signal.risk_pct_planned) : null,
    note: signal.note || null,
    rejectionReason: signal.rejection_reason || null,
    rawJson: jsonField(signal.raw_json),
    metadata: jsonField(signal.metadata),
  ...
  try {
    ...
    if (e.message?.includes("duplicate key") || e.code === "23505")
      return { sid: signal.sid, existed: true };
    throw e;
  }
}
...
async function findAccountByApiKeyHash(db, apiKeyHash) {
  const rows = await db
    .select()
    .from(schema.userAccounts)
    .where(eq(schema.userAccounts.apiKeyHash, apiKeyHash))
    .limit(1);
...
async function upsertUserAccount(db, userId, account) {
  ...
  const rows = await db
    ...
    .values({
      ...
      metadata: jsonField(account?.metadata),
      createdAt: now,
      updatedAt: now,
    })
    .onConflictDoUpdate({
...
async function deleteUserAccount(db, userId, accountId) {
  return db
    ...
    .where(
      and(
        eq(schema.userAccounts.userId, String(userId || "")),
        eq(schema.userAccounts.accountId, String(accountId || "")),
      ),
    );
}
...
async function listActiveSignals(db, userId) {
  return rowsSnake(
    await db
      ...
      .where(
        and(
          eq(schema.signals.userId, String(userId || "")),
...
async function getSignalByTicket(db, ticket) {
  const rows = await db
    .select()
    .from(schema.signals)
    .where(eq(schema.signals.sid, String(ticket || "")))
    .limit(1);
  return toSnake(rows[0]) || null;
}
...
async function bulkAckSignals(db, ids) {
  ...
  return db
    ...
    .set({ status: "ACKED" })
    .where(inArray(schema.signals.sid, ids));
}
...
async function findSignalById(db, id) {
  const rows = await db
    .select()
    .from(schema.signals)
    .where(eq(schema.signals.sid, String(id || "")))
    .limit(1);
  return toSnake(rows[0]) || null;
}
...
async function pruneOldSignals(db, days = 14) {
  ...
  return db
    ...
    .where(
      and(
        lte(schema.signals.createdAt, cutoff),
        eq(schema.signals.status, "CANCELLED"),
      ),
    );
}
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(schema.userSettings.type, type),
        eq(schema.userSettings.name, name || "default"),
      ),
    )
    .limit(1);
  return toSnake(rows[0]) || null;
...
async function getUserSettingData(db, userId, type, name) {
  const rows = await db
    .select({ data: schema.userSettings.data })
    .from(schema.userSettings)
    .where(
      and(
        eq(schema.userSettings.userId, userId),
  ...
  return db
    ...
    .onConflictDoUpdate({
      target: [
        ...
        schema.userSettings.type,
        schema.userSettings.name,
      ],
      set: {
...
async function closeSignal(db, sid) {
  return db
    ...
    .where(eq(schema.signals.sid, sid));
}

async function promoteDraftTrade(db, tradeRef, userId) {
  const c = [eq(schema.trades.sid, tradeRef)];
  if (userId) c.push(eq(schema.trades.userId, userId));
  return db
...
async function deleteUserTemplate(db, id) {
  return db
    ...
    .where(eq(schema.userTemplates.id, Number(id)));
}
...
async function getUserTemplateData(db, id) {
  const rows = await db
    .select({ data: schema.userTemplates.data })
    .from(schema.userTemplates)
    .where(eq(schema.userTemplates.id, Number(id)))
    .limit(1);
  return rows[0]?.data || null;
