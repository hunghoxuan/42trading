...
export function AdvancedOrderPanel({ accountId, initialSymbol = "", onOrderPlaced }) {
  ...
  const calculate = () => {
    ...
    if (!e || !s || Math.abs(e - s) === 0) {
       setResult({ units: metrics.min_vol, lots: metrics.min_vol / 100000, actualRisk: 0, rr: null, riskAmount });
       return;
    }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 2, background: 'rgba(255,255,255,0.03)', padding: 2, borderRadius: 6 }}>
           <button 
             ...
             style={{ flex: 1, height: 32, borderRadius: 4, background: side === 'BUY' ? '#10b981' : 'transparent', color: side === 'BUY' ? 'black' : 'var(--text)' }}
             onClick={() => setSide('BUY')}
           >
             BUY
           </button>
           <button 
             ...
             style={{ flex: 1, height: 32, borderRadius: 4, background: side === 'SELL' ? '#ef4444' : 'transparent', color: side === 'SELL' ? 'black' : 'var(--text)' }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
          <div className="cell-wrap">
            ...
            <input type="number" step="any" value={entry} onChange={e => setEntry(e.target.value)} placeholder={orderType === 'market' ? 'Auto' : ''} style={{ padding: '6px' }} />
          </div>
          <div className="cell-wrap">
          ...
          <div style={{ padding: 12, background: 'rgba(255,255,255,0.02)', borderRadius: 8, border: '1px solid var(--border)' }}>
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                   ...
                   <div style={{ fontSize: 20, fontWeight: 700, color: side === 'BUY' ? '#10b981' : '#ef4444' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                   ...
                   <div style={{ fontSize: 18, fontWeight: 600 }}>${result.actualRisk.toFixed(2)}</div>
                </div>
             </div>
             {result.rr && (
               <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', fontSize: 11 }}>
                  ...
                  <span style={{ fontWeight: 700 }}>1 : {result.rr.toFixed(2)}</span>
               </div>
             )}
          </div>
        )}
        ...
        <div className="cell-wrap">
           ...
           <textarea 
             ...
             style={{ height: 40, padding: 6, fontSize: 11, background: 'transparent', border: '1px solid var(--border)', borderRadius: 4 }}
           />
        </div>

        {error && <div className="msg-error" style={{ fontSize: 10 }}>{error}</div>}

        <button 
          className="primary-button" 
          disabled={!result || busy || !symbol}
          onClick={handlePlaceOrder}
          style={{ height: 40, fontSize: 14, fontWeight: 700, textTransform: 'uppercase' }}
        >
          {busy ? 'Processing...' : `Place ${side} ${orderType}`}
        </button>

        {metrics && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, opacity: 0.5, fontSize: 9 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
               <span>Spread: {metrics.spread}</span>
               <span>Pip Val: ${metrics.pip_value.toFixed(5)}</span>
            </div>
            {metrics.updated_at && <div>Broker sync: {new Date(metrics.updated_at).toLocaleTimeString()}</div>}
          </div>
        )}
      </div>
    </div>
