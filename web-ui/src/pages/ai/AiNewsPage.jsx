    ...
    <section className="news-calendar-page stack-layout fadeIn">
      ...
      <div className="news-calendar-panel">
        <div className="news-calendar-toolbar">
          <div className="news-calendar-tabs" role="tablist" aria-label="News range">
            {VIEW_OPTIONS.map((option) => (
              <button
                key={option.key}
                type="button"
                className={`secondary-button news-calendar-tab ${viewKey === option.key ? "active" : ""}`}
                onClick={() => setViewKey(option.key)}
              >
                {option.label}
              </button>
            ))}
